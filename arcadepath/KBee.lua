...
--------------------------------------------
    local match1234characters = {"SFZ_Dan", "SFZ_Adon", "SFZ_Guy", "SFZ_Sagat", "SFZ_Gen-Z", "SFZ_Sodom-Z", "SFZ_Rolento"}
    local match5characters = {"SFZ_Dhalsim"}
	local match6characters = {"SFZ_Charlie", "SFZ_Ryu", "SFZ_Ken"}
    local match7characters = {"SFZ_ChunLi-Z", "SFZ_Rose", "SFZ_Sakura"}
    local match8characters = {"SFZ_Akuma"}	
	local match9characters = {"SFZ_Vega"}
	local match10characters = {"SFZ_Bison"}

	launchStoryboard('data/Intros/Intro_KBee.def')
	launchStoryboard('data/Intros/Intro_KBee2.def')
...
if matchno() == 5 then
    ...
    local ok = launchFight{
        ...
        p2numchars = 1,
        p2teammode = "single",
		stage = "stages/_Boss-Dhalsim.def"
    }
