...
-- CHUNLI ARCADE PATH
--------------------------------------------
    local match1234characters = {"SFZ_Guile", "SFZ_Sakura", "SFZ_Zangief", "SFZ_FeiLong", "SFZ_Blanka", "SFZ_Rolento", "SFZ_Sodom-Z", "SFZ_Birdie", "SFZ_Mika", "SFZ_Dhalsim", "SFZ_Vega"}
    local match5characters = {"SFZ_Charlie"}
	local match6characters = {"SFZ_Guy", "SFZ_Ken"}
    local match7characters = {"SFZ_Ryu", "SFZ_Sagat"}
    local match8characters = {"SFZ_Gen-X"}	
	local match9characters = {"SFZ_KBee"}
	local match10characters = {"SFZ_N-Bison"}

	launchStoryboard('data/Intros/Intro_Chun_X.def')
	launchStoryboard('data/Intros/Intro_Chun_X2.def')
...
if matchno() == 9 then
    ...
    local ok = launchFight{
        ...
        p2numchars = 1,
        p2teammode = "single",
		stage = "stages/_Boss-KillerBee.def"
    }
