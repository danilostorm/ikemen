...
-- DHALSIM ARCADE PATH
--------------------------------------------
    local match1234characters = {"SFZ_Rolento", "SFZ_Ryu", "SFZ_Ken", "SFZ_Dan", "SFZ_Charlie", "SFZ_Birdie", "SFZ_Sakura", "SFZ_Gen-Z"}
    local match5characters = {"SFZ_Zangief"}
	local match6characters = {"SFZ_Adon", "SFZ_ChunLi-Z", "SFZ_Sodom-Z", "SFZ_Guy"}
    local match7characters = {"SFZ_Rose"}
    local match8characters = {"SFZ_Sagat"}	
	local match9characters = {"SFZ_Bison"}
	local match10characters = {"SFZ_Akuma"}

	launchStoryboard('data/Intros/Intro_Dhalsim.def')
	launchStoryboard('data/Intros/Intro_Dhalsim2.def')
...
if matchno() == 5 then
    ...
    local ok = launchFight{
        ...
        p2numchars = 1,
        p2teammode = "single",
		stage = "stages/_Boss-Zangief.def"
    }
