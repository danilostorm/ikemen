...
-- CHARLIE ARCADE PATH
--------------------------------------------
    local match1234characters = {"SFZ_ChunLi-Z", "SFZ_Sagat", "SFZ_Zangief", "SFZ_Dan", "SFZ_Rose", "SFZ_Gen-Z", "SFZ_Adon"}
    local match5characters = {"SFZ_Guile"}
	local match6characters = {"SFZ_Sodom-Z", "SFZ_Rolento", "SFZ_Birdie"}
    local match7characters = {"SFZ_Ryu", "SFZ_Akuma", "SFZ_Ken"}
    local match8characters = {"SFZ_Dhalsim"}	
	local match9characters = {"SFZ_Guy"}
	local match10characters = {"SFZ_Bison"}

	launchStoryboard('data/Intros/Intro_Charlie.def')
	launchStoryboard('data/Intros/Intro_Charlie2.def')
...
if matchno() == 10 then
    ...
    local ok = launchFight{
        ...
        p2numchars = 1,
        p2teammode = "single",
		stage = "stages/_Boss-Venezuela.def",
		music = {"Sound/Bison.mp3", 255, 396471, 2936422} 
    }
