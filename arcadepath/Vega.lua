...
-- VEGA ARCADE PATH
--------------------------------------------
    local match1234characters = {"SFZ_Blanka", "SFZ_Sagat", "SFZ_Karin", "SFZ_Sakura", "SFZ_Honda", "SFZ_Rolento", "SFZ_FeiLong", "SFZ_Ryu", "SFZ_Rose", "SFZ_Ken"}
    local match5characters = {"SFZ_Zangief"}
	local match6characters = {"SFZ_Akuma", "SFZ_ChunLi-X"}
    local match7characters = {"SFZ_Balrog", "SFZ_Guile", "SFZ_Birdie"}
    local match8characters = {"SFZ_Guy"}	
	local match9characters = {"SFZ_Mika"}
	local match10characters = {"SFZ_N-Bison"}

	launchStoryboard('data/Intros/Intro_Vega.def')
	launchStoryboard('data/Intros/Intro_Vega2.def')
...
if matchno() == 5 then
    ...
    local ok = launchFight{
        ...
        p2numchars = 1,
        p2teammode = "single",
		stage = "stages/_Boss-Zangief2.def"
    }
