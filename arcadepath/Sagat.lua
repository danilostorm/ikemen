...
-- SAGAT ARCADE PATH
--------------------------------------------
    local match1234characters = {"SFZ_Rose", "SFZ_ChunLi-Z", "SFZ_Akuma", "SFZ_Birdie", "SFZ_Guy"}
    local match5characters = {"SFZ_Adon"}
	local match6characters = {"SFZ_Balrog", "SFZ_Blanka", "SFZ_Gen-Z"}
    local match7characters = {"SFZ_Dhalsim", "SFZ_Sakura", "SFZ_Bison"}
    local match8characters = {"SFZ_Zangief"}	
	local match9characters = {"SFZ_Ken"}
	local match10characters = {"SFZ_Ryu"}

	launchStoryboard('data/Intros/Intro_Sagat.def')
	launchStoryboard('data/Intros/Intro_Sagat2.def')
...
if matchno() == 10 then
    ...
    local ok = launchFight{
        ...
        p2numchars = 1,
        p2teammode = "single",
		stage = "stages/_Boss-Outbacks.def"
    }
