...
-- BLANKA ARCADE PATH
--------------------------------------------
    local match1234characters = {"SFZ_DJ", "SFZ_Cody-X", "SFZ_Zangief", "SFZ_Karin", "SFZ_Kbee", "SFZ_T-Hawk"}
    local match5characters = {"SFZ_Dan"}
	local match6characters = {"SFZ_Dhalsim", "SFZ_Sodom-X", "SFZ_Balrog"}
    local match7characters = {"SFZ_ChunLi-X", "SFZ_Sagat", "SFZ_Vega"}
    local match8characters = {"SFZ_Honda"}	
	local match9characters = {"SFZ_Guile"}
	local match10characters = {"SFZ_D-Sakura"}

	launchStoryboard('data/Intros/Intro_Blanka.def')
	launchStoryboard('data/Intros/Intro_Blanka2.def')
...
if matchno() == 10 then
    ...
    local ok = launchFight{
        ...
        p2numchars = 1,
        p2teammode = "single",
		stage = "stages/_Boss-Sakura2.def"
    }
