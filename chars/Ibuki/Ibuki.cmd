;#ADD004BASIC PIEs#
;-| Button Remapping |-----------------------------------------------------
[Remap]
x = x
y = y
z = z
a = a
b = b
c = c
s = s
;-| Default Values |-------------------------------------------------------
[Defaults]
command.Time = 15
command.buffer.Time = 1


;-| Super Motions |--------------------------------------------------------
[Command]
name="YoroiDoushi"
command=~D, DF, F, D, DF, F, x
time=32
[Command]
name="YoroiDoushi"
command=~D, DF, F, D, DF, F, y
time=32
[Command]
name="YoroiDoushi"
command=~D, DF, F, D, DF, F, z
time=32
[Command]
name="YoroiDoushi"
command=~D, DF, F, D, DF, F, ~x
time=32
[Command]
name="YoroiDoushi"
command=~D, DF, F, D, DF, F, ~y
time=32
[Command]
name="YoroiDoushi"
command=~D, DF, F, D, DF, F, ~z
time=32

[Command]
name="RYoroiDoushi"
command=~D, DB, B, D, DB, B, x
time=32
[Command]
name="RYoroiDoushi"
command=~D, DB, B, D, DB, B, y
time=32
[Command]
name="RYoroiDoushi"
command=~D, DB, B, D, DB, B, z
time=32

[Command]
name="MaxYoroiDoushi"
command=~D, DF, F, D, DF, F, x+z
time=32
[Command]
name="MaxYoroiDoushi"
command=~D, DF, F, D, DF, F, x+y
time=32
[Command]
name="MaxYoroiDoushi"
command=~D, DF, F, D, DF, F, y+z
time=32

[Command]
name="MaxRYoroiDoushi"
command=~D, DB, B, D, DB, B, x+z
time=32
[Command]
name="MaxRYoroiDoushi"
command=~D, DB, B, D, DB, B, x+y
time=32
[Command]
name="MaxRYoroiDoushi"
command=~D, DB, B, D, DB, B, y+z
time=32

[Command]
name="HashinShou"
command=~D, DB, B, D, DB, B, x
time=32
[Command]
name="HashinShou"
command=~D, DB, B, D, DB, B, y
time=32
[Command]
name="HashinShou"
command=~D, DB, B, D, DB, B, z
time=32
[Command]
name="HashinShou"
command=~D, DB, B, D, DB, B, ~x
time=32
[Command]
name="HashinShou"
command=~D, DB, B, D, DB, B, ~y
time=32
[Command]
name="HashinShou"
command=~D, DB, B, D, DB, B, ~z
time=32

[Command]
name="MaxHashinShou"
command=~D, DB, B, D, DB, B, x+z
time=32
[Command]
name="MaxHashinShou"
command=~D, DB, B, D, DB, B, x+y
time=32
[Command]
name="MaxHashinShou"
command=~D, DB, B, D, DB, B, y+z
time=32

[Command]
name="YamiShigure"
command=~D, DF, F, D, DF, F, a
time=32
[Command]
name="YamiShigure"
command=~D, DF, F, D, DF, F, b
time=32
[Command]
name="YamiShigure"
command=~D, DF, F, D, DF, F, c
time=32
[Command]
name="YamiShigure"
command=~D, DF, F, D, DF, F, ~a
time=32
[Command]
name="YamiShigure"
command=~D, DF, F, D, DF, F, ~b
time=32
[Command]
name="YamiShigure"
command=~D, DF, F, D, DF, F, ~c
time=32

[Command]
name="MaxYamiShigure"
command=~D, DF, F, D, DF, F, a+b
time=32
[Command]
name="MaxYamiShigure"
command=~D, DF, F, D, DF, F, b+c
time=32
[Command]
name="MaxYamiShigure"
command=~D, DF, F, D, DF, F, a+c
time=32

[Command]
name="SaigoHashinShou"
command=~D, DB, B, D, DB, B, a+x
time=32
[Command]
name="SaigoHashinShou"
command=~D, DB, B, D, DB, B, b+y
time=32
[Command]
name="SaigoHashinShou"
command=~D, DB, B, D, DB, B, c+z
time=32
[Command]
name="SaigoHashinShou"
command=~D, DB, B, D, DB, B, ~a+x
time=32
[Command]
name="SaigoHashinShou"
command=~D, DB, B, D, DB, B, ~b+y
time=32
[Command]
name="SaigoHashinShou"
command=~D, DB, B, D, DB, B, ~c+z
time=32

[Command]
name="KachoFugetsu"
command=~D, DF, F, D, DF, F, a+x
time=32
[Command]
name="KachoFugetsu"
command=~D, DF, F, D, DF, F, b+y
time=32
[Command]
name="KachoFugetsu"
command=~D, DF, F, D, DF, F, c+z
time=32
[Command]
name="KachoFugetsu"
command=~D, DF, F, D, DF, F, ~a+x
time=32
[Command]
name="KachoFugetsu"
command=~D, DF, F, D, DF, F, ~b+y
time=32
[Command]
name="KachoFugetsu"
command=~D, DF, F, D, DF, F, ~c+z
time=32
;-| Special Motions |------------------------------------------------------

[Command]
name = "Hien"
command = ~B, D, DB,a
time = 18
[Command]
name = "Hien"
command = ~B, D, DB,b
time = 18
[Command]
name = "Hien"
command = ~B, D, DB,c
time = 18
[Command]
name = "Hien"
command = ~B, D, DB,~a
time = 18
[Command]
name = "Hien"
command = ~B, D, DB,~b
time = 18
[Command]
name = "Hien"
command = ~B, D, DB,~c
time = 18

[Command]
name = "HienEX"
command = ~B, D, DB,a+b
time = 18
[Command]
name = "HienEX"
command = ~B, D, DB,b+c
time = 18
[Command]
name = "HienEX"
command = ~B, D, DB,a+c
time = 18

[Command]
name = "Kazekiri"
command = ~F, D, DF, a
time = 18
[Command]
name = "Kazekiri"
command = ~F, D, DF, b
time = 18
[Command]
name = "Kazekiri"
command = ~F, D, DF, c
time = 18
[Command]
name = "Kazekiri"
command = ~F, D, DF, ~a
time = 18
[Command]
name = "Kazekiri"
command = ~F, D, DF, ~b
time = 18
[Command]
name = "Kazekiri"
command = ~F, D, DF, ~c
time = 18

[Command]
name = "KazekiriEX"
command = ~F, D, DF, a+b
time = 18
[Command]
name = "KazekiriEX"
command = ~F, D, DF, b+c
time = 18
[Command]
name = "KazekiriEX"
command = ~F, D, DF, a+c
time = 18

[Command]
name = "Kunai"
command = ~D, DF, F, x
time = 15
[Command]
name = "Kunai"
command = ~D, DF, F, y
time = 15
[Command]
name = "Kunai"
command = ~D, DF, F, z
time = 15
[Command]
name = "Kunai"
command = ~D, DF, F, ~x
time = 15
[Command]
name = "Kunai"
command = ~D, DF, F, ~y
time = 15
[Command]
name = "Kunai"
command = ~D, DF, F, ~z
time = 15

[Command]
name = "KunaiEX"
command = ~D, DF, F, x+z
time = 15
[Command]
name = "KunaiEX"
command = ~D, DF, F, y+z
time = 15
[Command]
name = "KunaiEX"
command = ~D, DF, F, x+y
time = 15

[Command]
name = "RKunai"
command = ~D, DB, B, x
time = 15
[Command]
name = "RKunai"
command = ~D, DB, B, y
time = 15
[Command]
name = "RKunai"
command = ~D, DB, B, z
time = 15

[Command]
name = "RKunaiEX"
command = ~D, DB, B, x+z
time = 15
[Command]
name = "RKunaiEX"
command = ~D, DB, B, y+z
time = 15
[Command]
name = "RKunaiEX"
command = ~D, DB, B, x+y
time = 15

[Command]
name = "Kasumigake1"
command = ~D, DF, F, a
time = 15
[Command]
name = "Kasumigake2"
command = ~D, DF, F, b
time = 15
[Command]
name = "Kasumigake3"
command = ~D, DF, F, c
time = 15
[Command]
name = "Kasumigake1"
command = ~D, DF, F, ~a
time = 15
[Command]
name = "Kasumigake2"
command = ~D, DF, F, ~b 
time = 15
[Command]
name = "Kasumigake3"
command = ~D, DF, F, ~c
time = 15

[Command]
name = "KasumigakeEX"
command = ~D, DF, F, a+b
time = 15
[Command]
name = "KasumigakeEX"
command = ~D, DF, F, b+c
time = 15
[Command]
name = "KasumigakeEX"
command = ~D, DF, F, a+c
time = 15

[Command]
name = "Kubiori"
command = ~$B, D, DF, F, x
time = 22
[Command]
name = "Kubiori"
command = ~$B, D, DF, F, y
time = 22
[Command]
name = "Kubiori"
command = ~$B, D, DF, F, z
time = 22
[Command]
name = "Kubiori"
command = ~B, DB, D, DF, $F, x
time = 22
[Command]
name = "Kubiori"
command = ~B, DB, D, DF, $F, y
time = 22
[Command]
name = "Kubiori"
command = ~B, DB, D, DF, $F, z
time = 22

[Command]
name = "KubioriEX"
command = ~$B, D, DF, F, x+y
time = 22
[Command]
name = "KubioriEX"
command = ~$B, D, DF, F, y+z
time = 22
[Command]
name = "KubioriEX"
command = ~$B, D, DF, F, x+z
time = 22

[Command]
name = "Tsumuji"
command = ~D, DB, B, a
time = 15
[Command]
name = "Tsumuji"
command = ~D, DB, B, b
time = 15
[Command]
name = "Tsumuji"
command = ~D, DB, B, c
time = 15
[Command]
name = "Tsumuji"
command = ~D, DB, B, ~a
time = 15
[Command]
name = "Tsumuji"
command = ~D, DB, B, ~b
time = 15
[Command]
name = "Tsumuji"
command = ~D, DB, B, ~c
time = 15

[Command]
name = "TsumujiEX"
command = ~D, DB, B, a+c
time = 15
[Command]
name = "TsumujiEX"
command = ~D, DB, B, b+c
time = 15
[Command]
name = "TsumujiEX"
command = ~D, DB, B, a+b
time = 15

[Command]
name = "Raida"
command = ~$F, D, DB, B, x
time = 22
[Command]
name = "Raida"
command = ~$F, D, DB, B, y
time = 22
[Command]
name = "Raida"
command = ~$F, D, DB, B, z
time = 22
[Command]
name = "Raida"
command = ~F, DF, D, DB, $B, x
time = 22
[Command]
name = "Raida"
command = ~F, DF, D, DB, $B, y
time = 22
[Command]
name = "Raida"
command = ~F, DF, D, DB, $B, z
time = 22

[Command]
name = "RaidaEX"
command = ~$F, D, DB, B, x+y
time = 22
[Command]
name = "RaidaEX"
command = ~$F, D, DB, B, y+z
time = 22
[Command]
name = "RaidaEX"
command = ~$F, D, DB, B, x+z
time = 22
;-------------------------------------------------------------
[Command]
name = "412p" ;Zero Counter
command = ~B, DB, D, x
time=32
[Command]
name = "412p" ;Zero Counter
command = ~B, DB, D, y
time=32
[Command]
name = "412p" ;Zero Counter
command = ~B, DB, D, z
time=32
[Command]
name = "412p" ;Zero Counter
command = ~B, DB, D, ~x
time=32
[Command]
name = "412p" ;Zero Counter
command = ~B, DB, D, ~y
time=32
[Command]
name = "412p" ;Zero Counter
command = ~B, DB, D, ~z
time=32
[Command]
name = "412k" ;Zero Counter
command = ~B, DB, D, a
time=32
[Command]
name = "412k" ;Zero Counter
command = ~B, DB, D, b
time=32
[Command]
name=  "412k" ;Zero Counter
command = ~B, DB, D, c
time=32
[Command]
name = "412k" ;Zero Counter
command = ~B, DB, D, ~a
time=32
[Command]
name = "412k" ;Zero Counter
command = ~B, DB, D, ~b
time=32
[Command]
name = "412k" ;Zero Counter
command = ~B, DB, D, ~c
time=32
;-| Double Tap |-----------------------------------------------------------
[Command]
name = "FF"     ;Required (do not remove)
command = F, F
time=10

[Command]
name = "BB"     ;Required (do not remove)
command = B, B
time=10

;-| 2/3 Button Combination |-----------------------------------------------
[Command]
name = "recovery";Required (do not remove)
command = x+y
time=1
[Command]
name= "recovery"
command =a+x
time=1
[Command]
name = "recovery"
command = y+z
time=1
[Command]
name = "recovery"
command = x+z
time=1
[Command]
name = "recovery"
command = a+b
time=1
[Command]
name = "recovery"
command = b+c
time=1
[Command]
name = "recovery"
command = a+c
time=1

[Command]
name = "xy"
command = x+y
time=1
[Command]
name = "xz"
command = x+z
time=1
[Command]
name = "yz"
command = y+z
time=1

[Command]
name = "pp"
command = x+y
time=1
[Command]
name = "pp"
command = x+z
time=1
[Command]
name = "pp"
command = y+z
time=1
[Command]
name = "kk"
command = a+b
time=1
[Command]
name = "kk"
command = a+c
time=1
[Command]
name = "kk"
command = b+c
time=1
[Command]
name = "a+x"
command = a+x
time=1
[Command]
name = "b+y"
command = b+y
time=1
[Command]
name = "c+z"
command = c+z
time=1
[Command]
name = "a+c"
command = a+c
time=1
[Command]
name = "a+b"
command = a+b
time = 1
[Command]
name = "b+c"
command = b+c
time = 1
;-| Single Button |---------------------------------------------------------
[Command]
name = "a"
command = a
time=1
[Command]
name = "b"
command = b
time=1
[Command]
name = "c"
command = c
time=1
[Command]
name = "x"
command = x
time=1
[Command]
name = "y"
command = y
time=1
[Command]
name = "z"
command = z
time=1
[Command]
name = "s"
command = s
time=1
;-| Single Dir |------------------------------------------------------------
[Command]
name = "fwd" ;Required (do not remove)
command = $F
time=1
[Command]
name = "downfwd"
command = $DF
time=1
[Command]
name = "down" ;Required (do not remove)
command = $D
time=1
[Command]
name = "downback"
command = $DB
time=1
[Command]
name = "back" ;Required (do not remove)
command = $B
time=1
[Command]
name = "upback"
command = $UB
time=1
[Command]
name = "up" ;Required (do not remove)
command = $U
time=1
[Command]
name = "upfwd"
command = $UF
time=1
;-| Hold Button |--------------------------------------------------------------
[Command]
name = "holdx"
command = /x
time=1
[Command]
name = "holdy"
command = /y
time=1
[Command]
name = "holdz"
command = /z
time=1
[Command]
name = "holda"
command = /a
time=1
[Command]
name = "holdb"
command = /b
time=1
[Command]
name = "holdc"
command = /c
time=1
[Command]
name = "holds"
command = /s
time=1
;-| Hold Dir |--------------------------------------------------------------
[Command]
name = "holdfwd";Required (do not remove)
command = /$F
time=1
[Command]
name = "holdback";Required (do not remove)
command = /$B
time=1
[Command]
name = "holdup" ;Required (do not remove)
command = /$U
time=1
[Command]
name = "holddown";Required (do not remove)
command = /$D
time=1
[Command]
name = "holdfwd" ;Required (do not remove)
command = /$F
time=1
[Command]
name = "holddownfwd"
command = /$DF
time=1
[Command]
name = "holddown" ;Required (do not remove)
command = /$D
time=1
[Command]
name = "holddownback"
command = /$DB
time=1
[Command]
name = "holdback" ;Required (do not remove)
command = /$B
time=1
[Command]
name = "holdupback"
command = /$UB
time=1
[Command]
name = "holdup" ;Required (do not remove)
command = /$U
time=1
[Command]
name = "holdupfwd"
command = /$UF
time=1
;---------------------------------------------------------------------------
;Release Direction
[Command]
name = "rlsfwd"
command = ~$F
time=1
[Command]
name = "rlsback"
command = ~$B
time=1
[Command]
name = "rlsup"
command = ~$U
time=1
[Command]
name = "rlsdown"
command = ~$D
time=1
;--------------------------------------------------------------------------
;Release Button
[Command]
name = "rlsx"
command = ~x
time=1
[Command]
name = "rlsy"
command = ~y
time=1
[Command]
name = "rlsz"
command = ~z
time=1
[Command]
name = "rlsa"
command = ~a
time=1
[Command]
name = "rlsb"
command = ~b
time=1
[Command]
name = "rlsc"
command = ~c
time=1
;---------------------------------------------------------------------------
;Other
[Command]
name = "highjump"
command = $D, $U
time=15
[Command]
name = "DU"
command = D, U
time=18
[Command]
name = "DU"
command = DB, UF
time=18
[Command]
name = "DU"
command = DF, UB
time=18
[Command]
name = "DU"
command = $D, UF
time=18
[Command]
name = "DU"
command = $D, UB
time=18
;---------------------------------------------------------------------------
[Statedef -1]

;//==========================
;// add004-1-sctrls-start
;//==========================
;--- partner_standby
[state 0]
	type=selfstate
	trigger1=ctrl && numpartner && !ishelper && roundstate=2 && pos y=0
	trigger1=sysfvar(4)>0 && sysfvar(0)>0 && playeridexist(floor(sysfvar(0)))
	trigger1=playerid(floor(sysfvar(0))),var(0)=90900 && playerid(floor(sysfvar(0))),var(22)=4
	value=190190
	ctrl=0
	ignorehitpause=1
;//==========================
;// add004-1-sctrls-end
;//==========================




[State -1, Tick Fix]
type = CtrlSet
triggerall = !ctrl && roundstate=2
trigger1 = (StateNo = 52 || StateNo = 101 || StateNo = 5120) && !AnimTime
trigger2 = (StateNo = [200,499]) && !AnimTime
trigger3 = StateNo = 810 && !AnimTime
trigger4 = (StateNo = 5001 || StateNo = 5011 || StateNo = 151 || StateNo = 153) && HitOver
trigger5 = (StateNo = [700,715]) && !AnimTime
trigger6 = (StateNo = [6080,6082]) && !AnimTime
value = 1
;-----------------------------------------------------------------
[State -1, Parry Stand]
type = HitOverride
triggerall =!AILevel && roundstate=2 && statetype!= A 
triggerall = command = "fwd" && command!= "back" && command != "up" && command != "down"
trigger1 = Ctrl
attr = SA,AA,AP
stateno = 6080
slot = 0
time = 5
;------------------------------------------------------------------
[State -1, Crouching Parry]
type = HitOverride
triggerall =!AILevel && roundstate=2 && statetype!= A 
triggerall = (statetype = S && command = "down")|| (statetype = C && command = "fwd") && command != "back" && command != "up"
trigger1 = Ctrl
attr = C,AA,AP
stateno = 6081
slot = 0
time =5
;-------------------------------------------------------------------
[State -1, Aerial Parry]
type= HitOverride
triggerall =!AILevel && roundstate=2 && statetype= A 
triggerall = (statetype= A && command = "fwd") && command != "back" && command != "up" && command != "down"
trigger1 = Ctrl
attr = SA,AA,AP
stateno = 6082
forceair = 1
slot = 0
time = 5
;======================================================================
[State -1, Kacho Fugetsu]
type=ChangeState
value=3500
triggerall=!AILevel && RoundState=2 && (stateno!= [3500,3510]) && StateType != A 
triggerall= var(20) <= 60 && power >= 3000 && command ="KachoFugetsu"
trigger1=ctrl || StateNo=40 || StateNo=52 || (StateNo=[100,103])
trigger2=var(6)|| var(7)
;----------------------------------------------------------------------
[State -1, Saigo Hashin Shou]
type=ChangeState
value=3400
triggerall=!AILevel && RoundState=2 && (stateno!= [3400,3410]) && StateType != A 
triggerall= var(20) <= 60 && power >= 3000 && command ="SaigoHashinShou"
trigger1=ctrl || StateNo=40 || StateNo=52 || (StateNo=[100,103])
trigger2=var(6)|| var(7)
;----------------------------------------------------------------------
[State -1, Max Hashin Shou]
type=ChangeState
value=3350
triggerall=!AILevel && RoundState=2 && (stateno!= 3350) && StateType != A 
triggerall= var(20) <= 60 && power >= 2000 && command ="MaxHashinShou"
trigger1=ctrl || StateNo=40 || StateNo=52 || (StateNo=[100,103])
trigger2=var(6)|| var(7)
;----------------------------------------------------------------------
[State -1, Hashin Shou]
type=ChangeState
value=3300
triggerall=!AILevel && RoundState=2 && (stateno!= 3300) 
triggerall=StateType != A && var(20) <= 60 && power >= 1000 && command ="HashinShou"
trigger1=ctrl || StateNo=40 || StateNo=52 || (StateNo=[100,103])
trigger2=var(6)|| var(7)
;----------------------------------------------------------------------
[State -1, Max Yami Shigure]
type=ChangeState
value=3250
triggerall=!AILevel && RoundState=2 && (stateno!= [3250,3270]) && StateType != A 
triggerall= var(20) <= 60 && power >= 2000 && command ="MaxYamiShigure"
trigger1=ctrl || StateNo=40 || StateNo=52 || (StateNo=[100,103])
trigger2=var(6)|| var(7)
;----------------------------------------------------------------------
[State -1, Yami Shigure]
type=ChangeState
value=3200
triggerall=!AILevel && RoundState=2 && !Numhelper(3205) && (stateno!= [3200,3220]) 
triggerall=StateType != A && var(20) <= 60 && power >= 1000 && command ="YamiShigure"
trigger1=ctrl || StateNo=40 || StateNo=52 || (StateNo=[100,103])
trigger2=var(6)|| var(7)
;----------------------------------------------------------------------
[State -1, Max YoroiDoushi]
type=ChangeState
value=3050
triggerall=!AILevel && RoundState=2 && (stateno!= [3050,3060]) && StateType != A 
triggerall= var(20) <= 60 && power >= 2000 && command ="MaxYoroiDoushi"
trigger1=ctrl || StateNo=40 || StateNo=52 || (StateNo=[100,103])
trigger2=var(6)|| var(7)
;----------------------------------------------------------------------
[State -1, Yoroi Doushi]
type=ChangeState
value=3000
triggerall=!AILevel && RoundState=2 && !Numhelper(3001) && (stateno!= [3000,3010]) 
triggerall=StateType != A && var(20) <= 60 && power >= 1000 && command ="YoroiDoushi"
trigger1=ctrl || StateNo=40 || StateNo=52 || (StateNo=[100,103])
trigger2=var(6)|| var(7)
;----------------------------------------------------------------------
[State -1, Max Kasumi Suzaku]
type=ChangeState
value=3150
triggerall=!AILevel && RoundState=2 && StateType = A && Pos y<=-15 
triggerall= var(20) <= 60 && power >= 2000 && Ifelse(enemynear,facing=-facing,command ="MaxYoroiDoushi",command ="MaxRYoroiDoushi")
trigger1=ctrl
trigger2=var(6)|| var(7)
trigger3=(stateno=[600,650]) && movecontact
trigger4=stateno=1410 && animelemtime(5)>0 
trigger5=stateno=1460 && Movecontact<0
trigger6=stateno=1600 && anim=1602 && animelemtime(4)>1
;----------------------------------------------------------------------
[State -1, Kasumi Suzaku]
type=ChangeState
value=3100
triggerall=!AILevel && RoundState=2 && stateno!= 3100 && !Numhelper(3105) && StateType = A 
triggerall= Pos y<=-15 && var(20) <= 60 && power >= 1000 && Ifelse(enemynear,facing=-facing,command ="YoroiDoushi",command ="RYoroiDoushi")
trigger1=ctrl
trigger2=var(6)|| var(7)
trigger3=(stateno=[600,650]) && movecontact
trigger4=stateno=1410 && animelemtime(5)>0 
trigger5=stateno=1460 && Movecontact
trigger6=stateno=1600 && anim=1602 && animelemtime(4)>1
;----------------------------------------------------------------------
[State -1, Hien]
type=ChangeState
value=Ifelse(command="HienEX" && var(20) <= 60 && power>=500,1450,1400)
triggerall=!AILevel && RoundState=2 && StateType != A && ((command="Hien")||(command="HienEX" && power>=500 && var(20) <= 60))
trigger1=ctrl || StateNo=40 || StateNo=52 || (StateNo=[100,101])
trigger2=var(5)
;----------------------------------------------------------------------
[State -1, Kazekiri]
type=ChangeState
value=Ifelse(command="KazekiriEX" && var(20) <= 60 && power>=500,1250,1200)
triggerall=!AILevel && RoundState=2 && StateType != A && ((command="Kazekiri")||(command="KazekiriEX" && power>=500 && var(20) <= 60))
trigger1=ctrl || StateNo=40 || StateNo=52 || (StateNo=[100,101])
trigger2=var(5)
;----------------------------------------------------------------------
[State -1, Raida]
type=ChangeState
value=Ifelse(command="RaidaEX" && var(20) <= 60 && power>=500,1550,1500)
triggerall=!AILevel && RoundState=2 && StateType != A && ((command="Raida")||(command="RaidaEX" && power>=500 && var(20) <= 60))
trigger1=ctrl || StateNo=40 || StateNo=52 || (StateNo=[100,101])
trigger2=var(5)
;----------------------------------------------------------------------
[State -1, Kiobori]
type=ChangeState
value=Ifelse(command="KubioriEX"&&var(20) <= 60&&power>=500,1150,1100)
triggerall=!AILevel && RoundState=2 && StateType != A && var(45)
triggerall= ((command="Kubiori")||(command="KubioriEX" && power>=500 && var(20) <= 60))
trigger1=ctrl || StateNo=40 || StateNo=52 || (StateNo=[100,103])
trigger2=var(5)
;----------------------------------------------------------------------
[State -1, Air Kunai]
type=ChangeState
value=Ifelse(command="KunaiEX"&&var(20) <= 60&&power>=500,1075,1050)
triggerall=!AILevel && RoundState=2 && StateType = A && Pos y<=-15 && Ifelse(!var(20),!Numhelper(1025),Numhelper(1025)<15)
triggerall= Ifelse(enemynear,facing=-facing,((command="Kunai")||(command="KunaiEX"&&power>=500&&var(20) <= 60)),((command="RKunai")||(command="RKunaiEX"&&power>=500&&var(20) <= 60)))
trigger1=ctrl
trigger2=var(5)
trigger3=(stateno=[600,650]) && movecontact
trigger4=stateno=1410 && animelemtime(5)>0 
trigger5=stateno=1460 && Movecontact
trigger6=stateno=1600 && anim=1602 && animelemtime(4)>1
;----------------------------------------------------------------------
[State -1, Kunai]
type=ChangeState
value=Ifelse(command="KunaiEX"&&var(20) <= 60&&power>=500,1025,1000)
triggerall=!AILevel && RoundState=2 && StateType != A && Ifelse(!var(20),!Numhelper(1005),Numhelper(1005)<15)
triggerall=var(45)=0
triggerall= ((command="Kunai")||(command="KunaiEX" && power>=500 && var(20) <= 60))
trigger1=ctrl || StateNo=40 || StateNo=52 || (StateNo=[100,103])
trigger2=var(5)
trigger3=(stateno = 450) && movecontact && (command="KunaiEX"&&power>=500&&var(20) <= 60)
;----------------------------------------------------------------------
[State -1, Tsumuji]
type=ChangeState
value=Ifelse(command="TsumujiEX" && var(20) <= 60 && power>=500,1350,1300)
triggerall=!AILevel && RoundState=2 && StateType != A && ((command="Tsumuji")||(command="TsumujiEX" && power>=500 && var(20) <= 60))
trigger1=ctrl || StateNo=40 || StateNo=52 || (StateNo=[100,101])
trigger2=var(5)
;----------------------------------------------------------------------
[State -1, Kasumigake]
type=ChangeState
value=1600
triggerall=!AILevel && RoundState=2 && StateType != A && (command="Kasumigake1"||command="Kasumigake2"||command="Kasumigake3")
trigger1=ctrl || StateNo=40 || StateNo=52 || (StateNo=[100,101])
trigger2=var(5)
;--------------------------------------------------------------------------
[State -1, SIdestep/Dodge]
type = ChangeState
value = Ifelse(command = "holdfwd",710,Ifelse(command = "holdback",715,700))
triggerall = command = "a+x"
triggerall = !AILevel && RoundState = 2 && StateType != A
trigger1 = (ctrl || (StateNo = [100,103]))
;========================================================================
[State -1, Zero Counter]
type = ChangeState
value = 750
trigger1 = StateNo = 150 || StateNo = 152
trigger1 = command = "412p" || command = "412k"
trigger1 = !AILevel && RoundState = 2 && StateType != A
trigger1 = power >= 1000 && !var(20)
;------------------------------------------------------------------------
[State -1, Throw]
type=ChangeState
value =800
trigger1=(command = "holdfwd"||command = "holdback")&&(command = "pp"||command = "kk")
trigger1=!AILevel && RoundState=2 && Statetype!=A && !var(20)
trigger1=ctrl
;------------------------------------------------------------------------
[State -1, Air Throw]
type=ChangeState
value =850
trigger1=(command="holdfwd"||command="holdback")&&(command="pp")
trigger1=!AILevel && RoundState=2 && Statetype=A && Pos Y<=-30
trigger1=ctrl
;-------------------------------------------------------------------------
[State -1, Power Charge]
type = ChangeState
value = 730
trigger1 = command = "holdb" && command = "holdy"
trigger1 = !AILevel && RoundState = 2 && StateType != A
trigger1 = power < const(data.power) && power < PowerMax && !var(20)
trigger1 = ctrl || (StateNo = [100,103])
;--------------------------------------------------------------------------
[State -1,  Jump]
type = ChangeState
value = 123
triggerall= !AILevel && var(55)=2 && roundstate=2 && statetype != A
trigger1 = ctrl && command = "DU" 
trigger2 = stateno=420 && movehit && command = "holdup"
;--------------------------------------------------------------------------
[State -1, Custom Combo]
type = ChangeState
value = Ifelse(StateType = A,905,900)
triggerAll = !AILevel && command = "c+z" && RoundState = 2 && power >= 1000 && !var(20)
trigger1 = ctrl || StateNo = 52 || (StateNo = [100,103])
trigger1 = !var(41)
;--------------------------------------------------------------------------
[State -1, Dash Back]
type = ChangeState
value = 105
triggerall = !AILevel && Roundstate=2 && statetype = S
trigger1 = command = "BB"
trigger1 = ctrl
;--------------------------------------------------------------------------
[State -1, Run /Dash]
type = ChangeState
value = Ifelse(var(47)=1,100,102)
triggerall = !AILevel && Roundstate=2 && statetype = S
trigger1 = command = "FF" && ctrl
;--------------------------------------------------------------------------
[State -1, Dash Cancel]
type = ChangeState
value = 102
triggerall = !AILevel && Roundstate=2 && statetype = S
triggerall = command = "FF"
trigger1 = stateno=103 && time>1
;--------------------------------------------------------------------------
[State -1, Standing Low Punch]
type=ChangeState
value=200
triggerall=!AILevel && Roundstate=2 && statetype != A && command != "holddown"&& command = "x"
trigger1=ctrl||stateno=[100,103]
trigger2=var(4)
trigger3=(stateno = 200 && anim!=205 || stateno = 400 || stateno = 430) && time >= 5 && !var(55)
trigger4=(stateno=[230,239]) && movecontact && var(55)
;---------------------------------------------------------------------------
[State -1, Standing Medium Punch]
type=ChangeState
value=210
triggerall=!AILevel && Roundstate=2 && statetype != A && command != "holddown"&& command = "y"
trigger1=ctrl||stateno=[100,103]
trigger2=var(4)
trigger3=((stateno=[200,209])||(stateno=[230,239])||(stateno=[400,409])||(stateno=[430,439])) && movecontact && var(55)
trigger4=(stateno=200) && movecontact && !var(55)
;---------------------------------------------------------------------------
[State -1, Standing High Punch]
type=ChangeState
value=220
triggerall=!AILevel && Roundstate=2 && statetype != A && command != "holddown"&& command = "z"
trigger1=ctrl||stateno=[100,103]
trigger2=var(4)
trigger3=((stateno=[200,219])||(stateno=[230,249])||(stateno=[400,419])||(stateno=[430,449])) && movecontact && var(55)
trigger4=(stateno=240) && movecontact && !var(55)
trigger5=((stateno=400)||(stateno=410)) && movehit && p2bodydist x<40
trigger6=(stateno=210) && movecontact 
;---------------------------------------------------------------------------
[State -1, Standing Low Kick]
type=ChangeState
value=230
triggerall=!AILevel && Roundstate=2 && statetype != A && command != "holddown"&& command = "a"
trigger1=ctrl||stateno=[100,103]
trigger2=var(4)
trigger3=(stateno = 200 && anim!=205 || stateno = 230 || stateno = 430) && time >= 5 && !var(55)
trigger4=(stateno=[200,209]) && movecontact && var(55)
trigger5=(stateno=210) && movecontact 
;---------------------------------------------------------------------------
[State -1, Standing Medium Kick]
type=ChangeState
value=240
triggerall=!AILevel && Roundstate=2 && statetype != A && command != "holddown"&& command = "b"
trigger1=ctrl||stateno=[100,103]
trigger2=var(4)
trigger3=((stateno=[200,219])||(stateno=[230,239])||(stateno=[400,419])||(stateno=[430,439])) && movecontact && var(55)
trigger4=stateno=210 && prevstateno!=200 && anim!=215 && movecontact 
trigger5=stateno=230 && anim=230 && movecontact && animelemtime(14)<0
;---------------------------------------------------------------------------
[State -1, Standing High Kick]
type=ChangeState
value=250
triggerall=!AILevel && Roundstate=2 && statetype != A && command != "holddown" && command = "c"
trigger1=ctrl||stateno=[100,103]
trigger2=var(4)
trigger3=((stateno=[200,249])||(stateno=[400,419])||(stateno=[430,449])) && movecontact && var(55)
trigger4=(stateno=410||stateno=450) && movecontact
trigger5=stateno=240 && anim=240 && movecontact && animelemtime(13)<0
;---------------------------------------------------------------------------
[State -1, Crouching Low Punch]
type=ChangeState
value=400
triggerall=!AILevel && Roundstate=2 && statetype != A && command = "holddown"&& command = "x"
trigger1=ctrl||stateno=[100,103]
trigger2=var(4)
trigger3=(stateno = 200 && anim!=205 || stateno = 400 || stateno = 430) && movecontact
;---------------------------------------------------------------------------
[State -1, Crouching Medium Punch]
type=ChangeState
value=410
triggerall=!AILevel && Roundstate=2 && statetype != A && command = "holddown"&& command = "y"
trigger1=ctrl||stateno=[100,103]
trigger2=var(4)
trigger3=((stateno=[200,209])||(stateno=[230,239])||(stateno=[400,409])||(stateno=[430,439])) && movecontact && var(55)
;---------------------------------------------------------------------------
[State -1, Crouching High Punch]
type=ChangeState
value=420
triggerall=!AILevel && Roundstate=2 && statetype != A && command = "holddown"&& command = "z"
trigger1=ctrl||stateno=[100,103]
trigger2=var(4)
trigger3=((stateno=[200,219])||(stateno=[230,249])||(stateno=[400,419])||(stateno=[430,449]))&& movecontact && var(55)
;---------------------------------------------------------------------------
[State -1, Crouching Low Kick]
type=ChangeState
value=430
triggerall=!AILevel && Roundstate=2 && statetype != A && command = "holddown"&& command = "a"
trigger1=ctrl||stateno=[100,103]
trigger2=var(4)
trigger3=(stateno = 200 && anim!=205 || stateno = 400 || stateno = 430) && movecontact 
;---------------------------------------------------------------------------
[State -1, Crouching Medium Kick]
type=ChangeState
value=440
triggerall=!AILevel && Roundstate=2 && statetype != A && command = "holddown"&& command = "b"
trigger1=ctrl||stateno=[100,103]
trigger2=var(4)
trigger3=((stateno=[200,219])||(stateno=[230,239])||(stateno=[400,419])||(stateno=[430,439])) && movecontact && var(55)
;---------------------------------------------------------------------------
[State -1, Crouching High Kick]
type=ChangeState
value=450
triggerall=!AILevel && Roundstate=2 && statetype != A && command = "holddown" && command = "c"
trigger1=ctrl||stateno=[100,103]
trigger2=var(4)
trigger3=((stateno=[200,249])||(stateno=[400,449])) && movecontact && var(55)
trigger4=stateno=240 && movecontact
trigger5=stateno=210 && anim=215 && movecontact
;---------------------------------------------------------------------------
[State -1, Jumping Low Punch]
type=ChangeState
value=600
triggerall=!AILevel && Roundstate=2 && statetype=A && command = "x"
trigger1=ctrl
trigger2=var(4)
;---------------------------------------------------------------------------
[State -1, Jumping Medium Punch]
type=ChangeState
value=610
triggerall=!AILevel && Roundstate=2 && statetype=A && command = "y"
trigger1=ctrl
trigger2=var(4)
trigger3=((stateno=[600,609])||(stateno=[630,639])) && movecontact && var(55)=2
;---------------------------------------------------------------------------
[State -1, Jumping High Punch]
type=ChangeState
value=620
triggerall=!AILevel && Roundstate=2 && statetype=A && command = "z"
trigger1=ctrl
trigger2=var(4)
trigger3=((stateno=[600,619])||(stateno=[630,649])) && movecontact && var(55)=2
;---------------------------------------------------------------------------
[State -1, Jumping Low Kick]
type=ChangeState
value=630
triggerall=!AILevel && Roundstate=2 && statetype=A && command = "a"
trigger1=ctrl
trigger2=var(4)
trigger3=(stateno=[600,609]) && movecontact && var(55)=2
;---------------------------------------------------------------------------
[State -1, Jumping Medium Kick]
type=ChangeState
value=640
triggerall=!AILevel && Roundstate=2 && statetype=A && command = "b"
trigger1=ctrl
trigger2=var(4)
trigger3=((stateno=[600,619])||(stateno=[630,639])) && movecontact && var(55)=2
trigger4=stateno=600 && movecontact
;---------------------------------------------------------------------------
[State -1, Jumping High Kick]
type=ChangeState
value=650
triggerall=!AILevel && Roundstate=2 && statetype=A && command = "c"
trigger1=ctrl
trigger2=var(4)
trigger3=(stateno=[600,649]) && movecontact && var(55)=2
trigger4=stateno=620 && movecontact
;---------------------------------------------------------------------------
[State -1, Taunt]
type = ChangeState
value = 195
triggerall = command = "s"
triggerall = !AILevel && Roundstate=2 && stateType != A
triggerall = StateNo != [450,699]
trigger1 = ctrl || (StateNo = [100,103])
trigger2 = var(5)
;--------------------------------------------------------------------------
;AI
;--------------------------------------------------------------------------

[State -1, Guarding Remove Var]
type=VarSet
trigger1= AILevel && roundstate=2
trigger1= var(28)
var(28)=0
ignorehitpause=1

[State -1, No Guarding Var]
type=VarSet
triggerall= AILevel && roundstate=2
trigger1= ((stateno=[6080,6082]) || (prevstateno=[6080,6082]))
trigger2= enemynear,name="Ryu" && enemynear,authorname="Phantom.of.the.Server" && (enemynear,stateno=3020 || enemynear,numhelper(3025))
trigger3= enemynear,name="Ryu" && enemynear,authorname="Phantom.of.the.Server" && (enemynear,stateno=3040 || enemynear,numhelper(3045) || enemynear,numhelper(3055))
trigger4= enemynear,name="Ryu" && enemynear,authorname="5theone/x_vyn_x" && (enemynear,stateno=[1600,1603])
trigger5= enemynear,name="Evil Ryu Hoshi" && enemynear,authorname="Vyn" && (enemynear,stateno=[1600,1603])
trigger6= enemynear,name="Orochi Gill" && enemynear,authorname="Shin_Lvl2_Akuma, Orochi Gill, SlayerGatsu" && enemynear,stateno=1420
trigger7= (enemynear,name="God Akuma" || enemynear,name="Orochi Shin Akuma" || enemynear,name="Another God Akuma") && enemynear,stateno=9980
trigger8= enemynear,name="Another God Akuma" && enemynear,authorname="Phantom.of.the.Server"
trigger8= (enemynear,stateno=2210 || enemynear,stateno=9800 || (enemynear,stateno=[78954,78995]))
trigger9= enemynear,hitdefattr=SCA,AT
trigger10= (prevstateno=[700,715])
var(28)=1
ignorehitpause=1

[State -1, No Guarding]
type=AssertSpecial
trigger1= AILevel && roundstate=2
trigger1= var(28)
flag=nostandguard
flag2=nocrouchguard
flag3=noairguard
ignorehitpause=1

[State -1, Stop Guarding]
type=ChangeState
triggerall= AILevel && roundstate=2
triggerall= stateno=[120,130]
trigger1= var(28)
value=140
ignorehitpause=1

[State -1, DCEA]
type=VarSet
triggerall= AILevel && roundstate=2
trigger1= var(51)
var(51)=0
ignorehitpause=1

[State -1, DCEA]
type=VarSet
triggerall= AILevel && roundstate=2
trigger1= enemynear,movetype=A && (enemynear,stateno=[1000,4999]) && enemynear,animtime<=-30
var(51)=1
ignorehitpause=1

[State -1, DWMTCETOSM]
type=VarSet
triggerall= AILevel && roundstate=2
trigger1= var(52)
var(52)=0
ignorehitpause=1

[State -1, DWMTCETOSM]
type=VarSet
triggerall= AILevel && roundstate=2
trigger1= enemynear,statetype=S && enemynear,authorname="Phantom.of.the.Server" && enemynear,stateno=1400
trigger2= enemynear,name="Psylocke" && enemynear,authorname="DivineWolf" && (enemynear,stateno=[1500,1510])
trigger3= enemynear,name="Ryu" && enemynear,authorname="5theone/x_vyn_x" && (enemynear,stateno=[1600,1603])
trigger4= enemynear,name="Evil Ryu Hoshi" && enemynear,authorname="Vyn" && (enemynear,stateno=[1600,1603])
trigger5= enemynear,name="Another God Akuma" && enemynear,authorname="Phantom.of.the.Server"
trigger5= (enemynear,stateno=2210 || (enemynear,stateno=[4700,4722]) || enemynear,stateno=7318 || enemynear,stateno=9800 || enemynear,stateno=22050)
trigger6= enemynear,name="Orochi Shin Akuma" && enemynear,authorname="Phantom.of.the.Server"
trigger6= (enemynear,stateno=[2200,2222])
trigger7= enemynear,name="God Akuma" && enemynear,authorname="Plaza (Shadow Leo)"
trigger7= ((enemynear,stateno=[2200,2222]) || enemynear,stateno=1400)
var(52)=1
ignorehitpause=1


[State -1, AI Parry Stand]
type = HitOverride
triggerall = AILevel && statetype != A && roundstate=2
triggerall = Numenemy && random < (250 * (AIlevel ** 2 / 64.0))
triggerall = enemynear, movetype=A && inguarddist
trigger1 = ctrl
slot = 0
stateno = 6080
attr = SA, AA, AP
time = 3
;---------------------------------------------------------------
[State -1, AI Crouching Parry]
type = HitOverride
triggerall = AILevel && statetype != A && roundstate=2
triggerall = Numenemy && random < (250 * (AIlevel ** 2 / 64.0))
triggerall = enemynear, movetype=A && inguarddist
trigger1 = ctrl
slot = 0
stateno = 6081
attr = C, AA, AP
time = 3
;---------------------------------------------------------------
[State -1, AI Aerial Parry]
type = HitOverride
triggerall = AILevel && statetype = A && roundstate=2
triggerall = Numenemy && random < (250 * (AIlevel ** 2 / 64.0))
triggerall = enemynear, movetype=A && inguarddist
trigger1 = ctrl
slot = 0
stateno = 6082
attr = SCA, AA, AP
time = 3
;-------------------------------------------------------------------
;[State -1, Throw]
;type = ChangeState
;value = 800
;triggerall = AILevel && Random < (500 * (AIlevel ** 2 / 64.0))
;triggerall = roundstate=2 && statetype!=A && enemynear,statetype!=L&&(enemynear,stateno!=[5120,5201]) && !var(20)
;triggerall = enemynear,movetype!=H&&enemynear,statetype!=A &&enemynear,Hitover&&(p2bodydist x =[0,30])
;triggerall = ctrl||stateno=100||stateno=52
;trigger1 = random>=800
;trigger2 = enemynear,stateno=[120,155]
;trigger2 = random>=500
;trigger3 = stateno=100
;trigger4=(p2bodydist x=[0,30])&&random<250
;trigger5=(p2stateno=[120,155])&&(p2bodydist x=[0,30])&&random<500


[State -1, Throw]
type = ChangeState
value = 800
triggerall = AILevel
triggerall = p2life != 0
triggerall = ctrl = 1 
triggerall = statetype != A && (ctrl || stateno=21)
triggerall = p2statetype != A && p2statetype != L
triggerall = p2movetype != H
triggerall = random < 800
trigger1 = p2bodydist X < 30 && p2bodydist x > -10
trigger2 = stateno = 710 && animtime = 0
trigger2 = Random <= 850


[State -1, Air Throw]
type = ChangeState
value = 850
triggerall = AILevel && NumEnemy
triggerall = random < 800
trigger1 = RoundState = 2 && StateType = A
trigger1 = !var(16) && (var(15) < 1 || var(20))
trigger1 = ctrl && pos y <= -32
trigger1 = P2StateType = A && Random < (300 * (AILevel ** 2 / 64.0))
trigger1 = (P2Dist x = [-20,33]) && (P2Dist y = [-118,48])

;-------------------------------------------------------------------
;[State -1, Air Throw]
;type = ChangeState
;value = 850
;triggerall = AILevel && Random < (500 * (AIlevel ** 2 / 64.0)) && !var(20)
;triggerall = roundstate=2 && statetype=A&& enemynear,statetype!=L&&(enemynear,stateno!=[5120,5201])
;triggerall = enemynear,movetype!=H&&enemynear,statetype=A&&enemynear,Hitover&&(p2bodydist x =[0,30])&&(p2bodydist y =[-50,0])
;triggerall = ctrl
;trigger1 = random>=800
;trigger2 = enemynear,stateno=[120,155]
;trigger2 = random>=500
;trigger3=(p2bodydist x=[0,30])&&random<250
;trigger4=(p2stateno=[120,155])&&(p2bodydist x=[0,30])&&random<500
;------------------------------------------------------------------------
[State -1, Standing Low Punch AI]
type = ChangeState
value = 200
triggerall = AILevel && numenemy && roundstate=2 && stateType != A
triggerall = (p2bodydist x = [-30, 65])  && (p2bodydist y = [-70,5]) && P2statetype != C && P2statetype != L
trigger1 = (ctrl || stateno=21)&&random < 25
trigger2 = (stateno = [100,103]) && random < (300 * (AIlevel ** 2 / 64.0))
trigger3 = (stateno=[200,209]) && movehit && random < (400 * (AIlevel ** 2 / 64.0)) && !var(55)
;---------------------------------------------------------------------------
[State -1, Standing Medium Punch AI]
type = ChangeState
value = 210
triggerall = AILevel && numenemy && roundstate=2 && StateType != A && P2statetype != A 
triggerall = (p2bodydist x = [5, 60]) && p2statetype != L && !(enemynear, hitfall)
trigger1 = (ctrl || stateno=21) && random < (50 * (AIlevel ** 2 / 64.0))
trigger2 = ((stateno = [200,209])|| (stateno = [230,239])||(stateno = [400,409])||(stateno = [430,439]))&& movehit&&var(55)
trigger2 = random < 250
trigger3 = stateno=200 && movecontact && random<750
;---------------------------------------------------------------------------
[State -1, Standing High Punch AI]
type = ChangeState
value = 220
triggerall = AILevel && numenemy && roundstate=2 && StateType != A && P2statetype != C
triggerall = (p2bodydist x = [10, 80]) && (p2bodydist y = [ -80, 80]) && p2statetype != L && !(enemynear, hitfall)
trigger1 = (ctrl || stateno=21) && random < (50 * (AIlevel ** 2 / 64.0)) 
trigger2 = ((stateno = [210,219])|| (stateno = [240,249])||(stateno = [410,419])||(stateno = [440,449]))&& movehit&&var(55)
trigger2 = random < 750
trigger3 = (stateno=240) && movehit && anim=240 && random<300 && !var(55)
trigger4 = stateno=210 && movehit && random<750 
;---------------------------------------------------------------------------
[State -1, Standing Low Kick AI]
type = ChangeState
value = 230
triggerall = AILevel && numenemy && roundstate=2 && StateType != A && P2statetype != A
triggerall = (p2bodydist x = [0, 55]) && p2statetype != L && !(enemynear, hitfall)
trigger1 = (ctrl || stateno=21) && random < (25 * (AIlevel ** 2 / 64.0))
trigger2 = ((stateno = [200,209])|| (stateno = [400,409]))&& movehit&&var(55)
trigger2 = random < 200
trigger3 = (stateno = [100,103]) && random < 100
trigger4 =(stateno=[200,219]) && movecontact 
;---------------------------------------------------------------------------
[State -1, Standing Medium Kick AI]
type = ChangeState
value = 240
triggerall = AILevel && numenemy && roundstate=2 && StateType != A 
triggerall = (p2bodydist x = [0, 95]) && p2statetype != L && p2statetype != A && !(enemynear, hitfall)
trigger1 = (ctrl || stateno=21) && random < (25 * (AIlevel ** 2 / 64.0))
trigger2 = ((stateno = [210,219])|| (stateno = [230,239])||(stateno = [410,419])||(stateno = [430,439]))&& movehit&&var(55)
trigger2 = random < 150
trigger3 = p2statetype = C && (p2stateno=[120,150]) && ctrl
trigger4 = stateno=210 && prevstateno!=200 && anim!=215 && movecontact && random < 500 
trigger5 = stateno=230 && anim=230 && movehit && animelemtime(14)<0 && random < 500 
;---------------------------------------------------------------------------
[State -1, Standing High Kick AI]
type = ChangeState
value = 250
triggerall = AILevel && numenemy && roundstate=2 && StateType != A && P2statetype != C
triggerall = (p2bodydist x = [10, 120]) && (p2bodydist y = [ -60, 50]) && p2statetype != L 
trigger1 = (ctrl || stateno=21) && random < (50 * (AIlevel ** 2 / 64.0))
trigger2 =((stateno=[200,249])||(stateno=[400,419])||(stateno=[430,449])) && movecontact && var(55) && random < 400
trigger3 = (stateno=410||stateno=450) && movehit && random < 500 
trigger4 = stateno=240 && anim=240 && movecontact && random < 500 
trigger5 = stateno=250 && movehit && random<500 
;---------------------------------------------------------------------------
[State -1, Crouching Low Punch]
type = ChangeState
value = 400
triggerall = AILevel && numenemy && roundstate=2 && StateType != A
triggerall = (p2bodydist x = [-30, 75]) &&(p2bodydist y = [-50,25]) && P2statetype != A && P2statetype != L && !(enemynear, hitfall)
trigger1 = (ctrl || stateno=21) && random < (25 * (AIlevel ** 2 / 64.0))
trigger2 = stateno = [100,103]
trigger3=(stateno=[400,409]) && movecontact && random < 300 && !var(55)
;---------------------------------------------------------------------------
[State -1, Crouching Medium Punch]
type = ChangeState
value = 410
triggerall= AILevel && numenemy
triggerall= roundstate=2 && statetype!=A
triggerall= !var(16) && var(15)<1
triggerall= (p2bodydist x=[0,70]) && (p2bodydist y=[-50,50]) && !(enemynear,statetype=L) && !(enemynear,hitfall) && !(enemynear,statetype=A)
trigger1= (ctrl || stateno=21 || (stateno=[100,101])) && random<(100+50*(var(46)=0))*(AILevel**2/64.0)
trigger2= ctrl && stateno=6081 && p2bodydist x>=30 && !(enemynear,movetype=A) && random<250*(AILevel**2/64.0)
trigger3 = ((stateno = [200,209])|| (stateno = [230,239])||(stateno = [400,409])||(stateno = [430,439]))&& movehit&&var(55)
trigger3 = random < 800

;triggerall = AILevel && numenemy && roundstate=2 && StateType != A
;triggerall = (p2bodydist x = [35, 80]) &&(p2bodydist y = [-50,25]) && P2statetype != A && P2statetype != L && !(enemynear, hitfall)
;trigger1 = (ctrl || stateno=21) && random < (50 * (AIlevel ** 2 / 64.0))
;trigger2 = ((stateno = [200,209])|| (stateno = [230,239])||(stateno = [400,409])||(stateno = [430,439]))&& movehit&&var(55)
;trigger2 = random < 800
;---------------------------------------------------------------------------
[State -1, Crouching High Punch]
type = ChangeState
value = 420
triggerAll = AILevel && NumEnemy && var(20) <= 60
triggerAll = RoundState = 2 && Statetype != A
triggerAll = (P2BodyDist x = [0,35]) && (P2Dist y = [-135,0])
triggerAll = (EnemyNear, StateType = S) || ((EnemyNear, const(size.head.pos.y) <= -80) && P2StateType = C) || (EnemyNear, StateType = A)
trigger1 = (ctrl || (StateNo = [100,101]) || StateNo = 21) && Random < (250 * (AILevel ** 2 / 64.0))
trigger2 = ((stateno = [210,219])|| (stateno = [240,249])||(stateno = [410,419])||(stateno = [440,449]))&& movehit&&var(55)
trigger2 = random < 600

;triggerall = AILevel && numenemy && roundstate=2 && StateType != A
;triggerall = (p2bodydist x = [20, 60]) &&(p2bodydist y = [-80,5]) && P2statetype != L && !(enemynear, hitfall)
;trigger1 = (ctrl || stateno=21)&& random < (50 * (AIlevel ** 2 / 64.0))
;trigger2 = ((stateno = [210,219])|| (stateno = [240,249])||(stateno = [410,419])||(stateno = [440,449]))&& movehit&&var(55)
;trigger2 = random < 600
;---------------------------------------------------------------------------
[State -1, Crouching Low Kick]
type = ChangeState
value = 430
triggerall = AILevel && numenemy && roundstate=2 && StateType != A
triggerall = (p2bodydist x = [5, 75]) &&(p2bodydist y = [-50,25]) && P2statetype != A && P2statetype != L && !(enemynear, hitfall)
trigger1 = (ctrl || stateno=21) && random < (25 * (AIlevel ** 2 / 64.0))
trigger2 = ((stateno = [200,209])|| (stateno = [400,409]))&& movehit&&var(55)
;---------------------------------------------------------------------------
[State -1, Crouching Medium Kick]
type = ChangeState
value = 440
triggerall= AILevel && numenemy
triggerall= roundstate=2 && statetype!=A
triggerall= !var(16) && var(15)<1
triggerall= (p2bodydist x=[0,55]) && (p2bodydist y=[-50,50]) && !(enemynear,statetype=L) && !(enemynear,hitfall) && !(enemynear,statetype=A)
trigger1= (ctrl || stateno=21 || (stateno=[100,101])) && random<(100+50*(var(46)=0))*(AILevel**2/64.0)
trigger2 = ((stateno = [210,219])|| (stateno = [230,239])||(stateno = [410,419])||(stateno = [430,439]))&& movehit&&var(55)
trigger2 = random < 350

;triggerall = AILevel && numenemy && roundstate=2 && StateType != A
;triggerall = (p2bodydist x = [15, 115]) &&(p2bodydist y = [-110,-25]) && P2statetype != A && P2statetype != L && !(enemynear, hitfall)
;trigger1 = (ctrl || stateno=21) && random < (25 * (AIlevel ** 2 / 64.0))
;trigger2 = ((stateno = [210,219])|| (stateno = [230,239])||(stateno = [410,419])||(stateno = [430,439]))&& movehit&&var(55)
;trigger2 = random < 350
;---------------------------------------------------------------------------
[State -1, Crouching High Kick]
type = ChangeState
value = 450
triggerall= AILevel && numenemy
triggerall= roundstate=2 && statetype!=A
triggerall= !var(16) && var(15)<1
triggerall= (p2bodydist x=[0,70]) && (p2bodydist y=[-50,50]) && !(enemynear,statetype=A)
triggerall= !(enemynear,statetype=L) && !var(50)
trigger1= (ctrl || stateno=21 || (stateno=[100,101])) && !inguarddist && random<(100+50*(var(46)=0))*(AILevel**2/64.0)
trigger2= ctrl && (stateno=[6080,6081]) && p2bodydist x>=30 && !(enemynear,movetype=A) && random<200*(AILevel**2/64.0)
trigger3 = ((stateno = [220,229])|| (stateno = [240,249])||(stateno = [420,429])||(stateno = [440,449]))&& movehit&&var(55)
trigger3 = random < 900
trigger4 = stateno=240 && movehit && random<300
trigger5 = stateno=210 && anim=215 && random<400


;triggerall = AILevel && numenemy && roundstate=2 && StateType != A && P2statetype != A
;triggerall = (p2bodydist x = [30, 70]) && (p2bodydist y = [ -50, 50]) && p2statetype != L && p2statetype = S && !(enemynear, hitfall)
;trigger1 = (ctrl || stateno=21) && random < (100 * (AIlevel ** 2 / 64.0))
;trigger2 = ((stateno = [220,229])|| (stateno = [240,249])||(stateno = [420,429])||(stateno = [440,449]))&& movehit&&var(55)
;trigger2 = random < 900
;trigger4 = stateno=240 && movehit && random<300
;trigger5 = stateno=210 && anim=215 && random<400
;---------------------------------------------------------------------------
[State -1, Neutral Jumping Low Punch]
type = ChangeState
value = 600
triggerall= AIlevel && numenemy
triggerall= roundstate=2 && statetype=A && !vel x
triggerall= (p2bodydist x=[0,50]) && (p2bodydist y=[-50,50]) && !(enemynear,statetype=L) && !(enemynear,hitfall)
trigger1= ctrl && enemynear,statetype=A && vel y<=2 && random<25*(AIlevel**2/64.0)


[State -1, Angled Jumping Low Punch]
type = ChangeState
value = 605
triggerall= AIlevel && numenemy
triggerall= roundstate=2 && statetype=A && vel x
triggerall= (p2bodydist x=[0,50]) && (p2bodydist y=[-50,50]) && !(enemynear,statetype=L) && !(enemynear,hitfall)
trigger1= ctrl && enemynear,statetype=A && vel y<=2 && random<25*(AIlevel**2/64.0)

;triggerall = AILevel && numenemy&&roundstate=2 && statetype = A && (p2bodydist x = [5,70]) && (p2bodydist y = [ -140, -50]) && p2statetype != L
;trigger1 = ctrl && random < (100 * (AIlevel ** 2 / 64.0))
;---------------------------------------------------------------------------
[State -1, Neutral Jumping Medium Punch]
type = ChangeState
value = 610
triggerall= AILevel && numenemy
triggerall= roundstate=2 && statetype=A && !vel x
triggerall= !var(16) && var(15)<1
triggerall= (p2bodydist x=[0,80]) && (p2bodydist y=[-80,80]) && !(enemynear,statetype=L) && !(enemynear,hitfall)
trigger1= ctrl && !(enemynear,statetype=A) && (sysvar(1)=1 || sysvar(1)=0) && vel y>=-2
trigger1= p2bodydist x>=20 && (p2dist y=[-60,80]) && random<150*(AILevel**2/64.0)
trigger2= ctrl && enemynear,statetype=A && vel y<=2 && random<100*(AILevel**2/64.0)
trigger2 = (stateno = [600,609])&& movehit && var(55)=2 && random < 750
trigger3 = (stateno = [630,639])&& movehit && var(55)=2 && random < 250

[State -1, Angled Jumping Medium Punch]
type = ChangeState
value = 615
triggerall= AILevel && numenemy
triggerall= roundstate=2 && statetype=A && vel x
triggerall= !var(16) && var(15)<1
triggerall= (p2bodydist x=[0,80]) && (p2bodydist y=[-80,80]) && !(enemynear,statetype=L) && !(enemynear,hitfall)
trigger1= ctrl && !(enemynear,statetype=A) && (sysvar(1)=1 || sysvar(1)=0) && vel y>=-2
trigger1= p2bodydist x>=20 && (p2dist y=[-60,80]) && random<150*(AILevel**2/64.0)
trigger2= ctrl && enemynear,statetype=A && vel y<=2 && random<100*(AILevel**2/64.0)
trigger2 = (stateno = [600,609])&& movehit && var(55)=2 && random < 750
trigger3 = (stateno = [630,639])&& movehit && var(55)=2 && random < 250

;triggerall = AILevel && numenemy&&roundstate=2 && statetype = A && (p2bodydist x = [10, 100]) && (p2bodydist y = [ -75, 70]) && p2statetype != L 
;trigger1 = ctrl && random < (ifelse((vel x > 0 && p2statetype = A), 350, 150) * (AIlevel ** 2 / 64.0)) 
;trigger2 = (stateno = [600,609])&& movehit && var(55)=2 && random < 750
;trigger3 = (stateno = [630,639])&& movehit && var(55)=2 && random < 250
;---------------------------------------------------------------------------
[State -1, Jumping High Punch]
type = ChangeState
value = 620
triggerall= AILevel && numenemy
triggerall= roundstate=2 && statetype=A
triggerall= !var(16) && var(15)<1
triggerall= (p2bodydist x=[0,70]) && (p2bodydist y=[-80,80]) && !(enemynear,statetype=L) && !(enemynear,hitfall)
trigger1= ctrl && !(enemynear,statetype=A) && (sysvar(1)=1 || sysvar(1)=0) && vel y>=-2
trigger1= p2bodydist x>=20 && (p2dist y=[-60,80]) && random<350*(AILevel**2/64.0)
trigger2= ctrl && enemynear,statetype=A && vel y<=2 && random<200*(AILevel**2/64.0)
trigger3 = (stateno = [610,619])&& movehit && var(55)=2 && random < 700
trigger4 = (stateno = [640,649])&& movehit && var(55)=2 && random < 200



;triggerall = AILevel && numenemy&&roundstate=2 && statetype = A && (p2bodydist x = [30, 120]) && (p2bodydist y = [ -95, 90]) && p2statetype != L 
;trigger1 = ctrl && random < (100 * (AIlevel ** 2 / 64.0)) && !(enemynear, hitfall)
;trigger2 = (stateno = [610,619])&& movehit && var(55)=2 && random < 700
;trigger3 = (stateno = [640,649])&& movehit && var(55)=2 && random < 200
;---------------------------------------------------------------------------
[State -1, Neutral Jumping Low Kick]
type = ChangeState
value = 630
triggerall = AILevel && numenemy&&roundstate=2 && statetype = A && !vel x && (p2bodydist x = [5, 60]) && (p2bodydist y = [ -60, -15]) && p2statetype != L 
trigger1 = ctrl && random < (50 * (AIlevel ** 2 / 64.0)) && !(enemynear, hitfall)
trigger2 = (stateno = [600,609])&& movehit && var(55)=2 && random < 250


[State -1,Angled Jumping Low Kick]
type = ChangeState
value = 635
triggerall = AILevel && numenemy&&roundstate=2 && statetype = A && vel x && (p2bodydist x = [5, 60]) && (p2bodydist y = [ -60, -15]) && p2statetype != L 
trigger1 = ctrl && random < (50 * (AIlevel ** 2 / 64.0)) && !(enemynear, hitfall)
trigger2 = (stateno = [600,609])&& movehit && var(55)=2 && random < 250


;triggerall = AILevel && numenemy&&roundstate=2 && statetype = A && (p2bodydist x = [0, 70]) && (p2bodydist y = [ -60, 50]) && p2statetype != L 
;trigger1 = ctrl && random < (150 * (AIlevel ** 2 / 64.0))
;trigger2 = (stateno = [600,609])&& movehit && var(55)=2 && random < 250
;---------------------------------------------------------------------------
[State -1,Neutral Jumping Medium Kick]
type = ChangeState
value = 640
triggerall= AILevel && numenemy
triggerall= roundstate=2 && statetype=A && !vel x 
triggerall= !var(16) && var(15)<1
triggerall= (p2bodydist x=[0,80]) && (p2bodydist y=[-80,80]) && !(enemynear,statetype=L) && !(enemynear,hitfall)
trigger1= ctrl && !(enemynear,statetype=A) && (sysvar(1)=1 || sysvar(1)=0) && vel y>=-2
trigger1= p2bodydist x>=20 && (p2dist y=[-60,80]) && random<250*(AILevel**2/64.0)
trigger2= ctrl && enemynear,statetype=A && vel y<=2 && random<100*(AILevel**2/64.0)
trigger3 = (stateno = [610,619])&& movehit && var(55)=2 && random < 100
trigger4 = (stateno = [630,639])&& movehit && var(55)=2 && random < 750


[State -1, Angled Jumping Medium Kick]
type = ChangeState
value = 645
triggerall= AILevel && numenemy
triggerall= roundstate=2 && statetype=A && vel x 
triggerall= !var(16) && var(15)<1
triggerall= (p2bodydist x=[0,80]) && (p2bodydist y=[-80,80]) && !(enemynear,statetype=L) && !(enemynear,hitfall)
trigger1= ctrl && !(enemynear,statetype=A) && (sysvar(1)=1 || sysvar(1)=0) && vel y>=-2
trigger1= p2bodydist x>=20 && (p2dist y=[-60,80]) && random<250*(AILevel**2/64.0)
trigger2= ctrl && enemynear,statetype=A && vel y<=2 && random<100*(AILevel**2/64.0)
trigger3 = (stateno = [610,619])&& movehit && var(55)=2 && random < 100
trigger4 = (stateno = [630,639])&& movehit && var(55)=2 && random < 750
trigger5 = stateno=600 && movehit && var(55)!=2 && random < 500

;triggerall = AILevel && numenemy&&roundstate=2 && statetype = A && (p2bodydist x = [-20, 110]) && (p2bodydist y = [ -60, -15]) && p2statetype != L 
;trigger1 = ctrl && random < (350 * (AIlevel ** 2 / 64.0)) && !(enemynear, hitfall)
;trigger2 = (stateno = [610,619])&& movehit && var(55)=2 && random < 100
;trigger3 = (stateno = [630,639])&& movehit && var(55)=2 && random < 750
;trigger4 = stateno=600 && movehit && var(55)!=2 && random < 500
;---------------------------------------------------------------------------
[State -1, Jumping High Kick]
type = ChangeState
value = 650
triggerall= AILevel && numenemy
triggerall= roundstate=2 && statetype=A
triggerall= !var(16) && var(15)<1
triggerall= (p2bodydist x=[0,80]) && (p2bodydist y=[-80,80]) && !(enemynear,statetype=L) && !(enemynear,hitfall)
trigger1= ctrl && !(enemynear,statetype=A) && (sysvar(1)=1 || sysvar(1)=0) && vel y>=-2
trigger1= p2bodydist x>=20 && (p2dist y=[-60,80]) && random<350*(AILevel**2/64.0)
trigger2= ctrl && enemynear,statetype=A && vel y<=2 && random<200*(AILevel**2/64.0)
trigger3 = (stateno = [610,619])&& movehit && var(55)=2 && random < 250
trigger4 = (stateno = [640,649])&& movehit && var(55)=2 && random < 750
trigger5 = stateno=620 && movehit && var(55)!=2 && random < 500


;triggerall = AILevel && numenemy &&roundstate=2 && statetype = A && (p2bodydist x = [20, 140]) && (p2bodydist y = [ -60, 30]) && p2statetype != L 
;trigger1 = ctrl && random < (200 * (AIlevel ** 2 / 64.0)) && !(enemynear, hitfall) 
;trigger2 = (stateno = [610,619])&& movehit && var(55)=2 && random < 250
;trigger3 = (stateno = [640,649])&& movehit && var(55)=2 && random < 750
;trigger4 = stateno=620 && movehit && var(55)!=2 && random < 500
;---------------------------------------------------------------------------

[State -1, Walk]
type=ChangeState
value=21
triggerall= AILevel  && numenemy
triggerall= roundstate=2 && statetype=S && !inguarddist && anim!=5
triggerall= !((stateno=[10,12]) || stateno=21 || (stateno=[100,101]) || (stateno=[6080,6081]))
trigger1= ctrl && p2bodydist x>=75 && random<500*(AILevel**2/64.0)

[State -1, run]
type = changestate
value = Ifelse(var(47)=1,100,102)
triggerall = AIlevel && numenemy
triggerall = statetype = S && roundstate = 2
triggerall = (stateno != [100, 102]) && enemynear, movetype != A && p2bodydist x > 120
trigger1 = (ctrl || stateno=21) && random < (200 * (AIlevel ** 2 / 64.0))
trigger2 = var(47)=0 && stateno = 103 && time>1

[State -1, dash]
type = changestate
value = 105
triggerall = AIlevel && numenemy
triggerall = statetype = S && roundstate = 2 && (ctrl || stateno=21)
triggerall = (p2bodydist x = [0, 80]) && backedgebodydist > 80 && (stateno != [100, 105])
trigger1 = enemynear, movetype = A && random < (150 * (AIlevel ** 2 / 64.0))
trigger2 = enemynear, stateno = 5120 && enemynear, animtime= -3 && random < (300 * (AIlevel ** 2 / 64.0))


[State -1, Jump]
type = changestate
value = 40
triggerall= AILevel && numenemy
triggerall= roundstate=2 && statetype!=A && (ctrl || stateno=21)
triggerall= enemynear,p2dist x>=0 || backedgebodydist<=80
trigger1= enemynear,movetype=A && enemynear,vel x>=-1 && p2bodydist x<=120
trigger1= enemynear,hitdefattr=SC,ST,HT && random<500*(AILevel**2/64.0)
trigger2= enemynear,movetype=A && ((enemynear,stateno=[1000,1999]) || (enemynear,stateno=[3000,3999]))
trigger2= p2bodydist x>=120 && (((enemynear,numhelper) && !numhelper(9741)) || (enemynear,numproj))
trigger2= !(enemynear,statetype=A) && random<500*(AILevel**2/64.0)
trigger3= enemynear,statetype=A && (p2bodydist x=[40,60]) && (p2dist y=[-110,-80])
trigger3= enemynear,vel x>=0 && !(enemynear,movetype=A) && random<50*(AILevel**2/64.0)
trigger4= enemynear,name="God Akuma" || enemynear,name="Orochi Shin Akuma" || enemynear,name="Another God Akuma"
trigger4= enemynear,movetype=A && enemynear,stateno=9980 && p2bodydist x>=90 && random<500*(AILevel**2/64.0)

;[State -1, Jump]
;type = changestate
;value = 40
;trigger1 = AIlevel && numenemy && random < (350 * (AIlevel ** 2 / 64.0))
;trigger1 = roundstate = 2 && statetype != A && ctrl
;trigger1 = enemynear, movetype = A && p2bodydist x < 120 && enemynear, hitdefattr = SC, AT

[state -1,AI Air Guard]
type = ChangeState
value = 132
triggerall = AIlevel && numenemy&& roundstate = 2&&InGuardDist
triggerall = ctrl&&statetype = A
trigger1 = enemynear,numproj
trigger2 = enemynear,HitDefAttr = SCA, NA,SA,HA
trigger2 = random <=ifelse(backedgedist<=10,950,900)

[State -1, Guard]
type=ChangeState
value=120
triggerall= AILevel && numenemy
triggerall= roundstate=2 && inguarddist
triggerall= enemynear,p2dist x>=0 && !(stateno=[120,155]) && !var(20) && !var(26) && !var(28)
trigger1= (ctrl || stateno=0 || stateno=21 || (stateno=[100,101]))
trigger1= !(enemynear,hitdefattr=SCA,AT) && (enemynear,time<=120)
trigger1= statetype!=A || enemynear,statetype=A
trigger1= ifelse(statetype=A,((var(3)!=[1,2]) || stateno=5210),1)
trigger1= random<((500+ifelse((enemynear,stateno=[200,699]),333,ifelse((enemynear,stateno=[1000,2999]),666,1000))))*(AILevel**2/64.0)

[State -1, Guard]
type = ChangeState
value = 120
triggerall= AILevel && numenemy&& (StateNo!=[120,155]) && !(enemynear,ctrl) && random < (750 * (AIlevel ** 2 / 64.0))
triggerall= Ctrl||stateno = 21
triggerall=enemynear,Movetype=A && !(enemynear,hitdefattr=SCA,AT) 
trigger1 = inguarddist

[State -1,  Jump]
type = ChangeState
value = 123
triggerall = AILevel && numenemy && var(55)=2 && statetype != A && roundstate = 2 && (p2bodydist y = [-320,5])
triggerall = stateno != 100 && pos y = 0 && (enemynear,Statetype!=C)
trigger1 = ctrl && enemy,vel y < -1 && (p2bodydist x = [ 10, 70]) && enemynear,movetype != A && random < (200 * (AIlevel ** 2 / 64.0))
trigger2 = ctrl && enemy,vel y < -1 && random < (150 * (AIlevel ** 2 / 64.0))
trigger2 = enemynear,movetype = H && (enemynear,stateno=5040)
trigger2 = p2bodydist x <= 50 && random < (150 * (AIlevel ** 2 / 64.0))
trigger3 = ctrl && enemynear,MoveType != H&&P2BodyDist Y < -90&& enemy,vel y <= 0 && random < (350 * (AIlevel ** 2 / 64.0))
trigger4 = stateno=420 && anim=425 && MoveHit && random < (800 * (AIlevel ** 2 / 64.0))
trigger5 = ctrl && (p2bodydist x<=140) && (enemynear,statetype!=A) && (enemynear,movetype=A) && (enemynear,stateno=[2000,4999]) && random < (750 * (AIlevel ** 2 / 64.0))

[State -1, Zero Counter]
type = changestate
value = 750
trigger1 = AIlevel && numenemy
trigger1 = (p2dist x = [-90, 90]) && stateno = 150 || stateno = 152
trigger1 = roundstate = 2 && power >= 2000 && !var(20) && life < 500 && random < (10 * (AIlevel ** 2 / 64.0))

;[State -1, powercharge]
;type = changestate
;value = 730
;trigger1 = AIlevel && numenemy
;trigger1 = roundstate = 2 && statetype != A && ctrl
;trigger1 = power < const(data.power) && power < powermax && !var(20)
;trigger1 = random < (50 * (AIlevel ** 2 / 64.0)) && !inguarddist && p2movetype != A && p2dist x >= 160

[State -1, Power Charge]
type = ChangeState
value = 730
triggerAll = AILevel && NumEnemy
triggerAll = RoundState = 2 && StateType != A
triggerAll = Power < const(data.power) && !var(20) && PrevStateNo != 5120
trigger1 = (ctrl || stateno = 21) && Power < const(data.power) && Power < PowerMax && !var(20) && numpartner = 0
trigger1 = !InGuardDist && P2BodyDist x >= 160 && Random < (100 * (AILevel ** 2 / 64.0))
trigger2 = (ctrl || stateno = 21) && Power < const(data.power) && Power < PowerMax && !var(20) && numpartner >= 1 && (partner, stateno != [5000,5050])
trigger2 = !InGuardDist && P2BodyDist x >= 160 && Random < (100 * (AILevel ** 2 / 64.0))


[State -1, roll forward]
type = changestate
value = 710
triggerall= AIlevel && numenemy
triggerall= roundstate=2 && statetype!=A && !var(20) && !var(28)
triggerall= (facing=1 && (enemynear,facing=-1)) || (facing=-1 && (enemynear,facing=1))
triggerall= (ctrl || stateno=21 || (stateno=[100,103])) && !(enemy,hitdefattr=SC,ST,HT,AT)
trigger1= enemynear,movetype=A && (p2bodydist x=[80,120]) && random<400*(AIlevel**2/64.0)
trigger2= ((enemynear,stateno>=1000 && enemynear,animtime<=-30) || (enemynear,numproj) || (enemynear,numhelper)) && p2bodydist x>=80 && inguarddist && random<350*(AIlevel**2/64.0)

[State -1, roll backward]
type=changestate
value=715
triggerall= AIlevel && numenemy
triggerall= roundstate=2 && statetype!=A 
triggerall= !(enemy,hitdefattr=SC,AT) && !var(20)
triggerall= backedgebodydist>=80 && !(enemy,hitdefattr=SC,ST,HT,AT)
triggerall= !(stateno=[6080,6081]) && !(numtarget(1520)) && !(enemynear,stateno=5300)
triggerall= !numtarget(3000) && !numtarget(3010) && !numtarget(3060) && !(enemynear,stateno=[1110000,1115120])
triggerall= (ctrl ||stateno=21 || (stateno=[100,103])) && (p2bodydist x=[25,50])
trigger1= enemynear,movetype=A && random<100*(AIlevel**2/64.0)
trigger2= enemynear,statetype=L && random<150*(AIlevel**2/64.0)

[State -1, dodge]
type=changestate
value=700
triggerall= AIlevel && numenemy
triggerall= roundstate=2 && statetype!=A && !var(28)
triggerall= (facing=1 && (enemynear,facing=-1)) || (facing=-1 && (enemynear,facing=1))
triggerall= (ctrl ||stateno=21 || (stateno=[100,103])) && !(enemy,hitdefattr=SC,ST,HT,AT)
trigger1= enemynear,movetype=A && (p2bodydist x=[0,40]) && !var(15) && random<200*(AIlevel**2/64.0)
trigger2= ((enemynear,numproj) || (enemynear,numhelper)) && inguarddist && p2bodydist x>=50 && random<200*(AIlevel**2/64.0)

[State -1, Dash Backward]
type=ChangeState
value=105
triggerall= AILevel && numenemy
triggerall= roundstate=2 && statetype=S && var(20)<=150 && !var(26)
triggerall= (ctrl || stateno=21) && !var(51) && !(enemy,hitdefattr=SC,ST,HT,AT)
triggerall= (p2bodydist x=[-10,80]) && backedgebodydist>=80 && !(stateno=[100,106]) && !(prevstateno=[100,106])
triggerall= !((prevstateno=[6080,6082]) || (stateno=[6080,6082]))
trigger1= enemynear,movetype=A && p2bodydist x>=40 && random<200*(AILevel**2/64.0)
trigger2= (enemynear,stateno=[5100,5120]) || (enemynear,stateno=[5200,5201])
trigger2= p2bodydist x<=25 && random<150*(AILevel**2/64.0)


[State -1, airrecover]
type=changestate
value=ifelse((pos y>=-20),5200,5210)
triggerall= AILevel && numenemy
triggerall= roundstate=2 && stateno=5050
trigger1= vel y>-1 && alive && canrecover && random < (550 * (AIlevel ** 2 / 64.0))
;======================================================================
[State -1, Kacho Fugetsu AI]
type=ChangeState
value=3500
triggerall=AILevel && numenemy && RoundState=2 && (stateno!=[3500,3510]) && StateType != A 
triggerall=(enemynear,statetype != L) && (enemynear,stateno!=[5100,5220]) && (enemynear,movetype!=A)
triggerall=(p2bodydist x =[15,80]) && (enemynear,statetype!=A) && var(20)<=60 && power >= 3000 && random < (400 * (AIlevel ** 2 / 64.0))
trigger1=ctrl || StateNo=40 || StateNo=52 ||stateno=21 || (StateNo=[100,103])
trigger2=var(6)|| var(7)
;----------------------------------------------------------------------
[State -1, Saigo Hashin Shou AI]
type=ChangeState
value=3400
triggerall=AILevel && numenemy && RoundState=2 && (stateno!=[3400,3410]) && StateType != A 
triggerall=(enemynear,statetype != L) && (enemynear,stateno!=[5100,5220]) && (enemynear,stateno!=[120,155]) && ((p2bodydist y=[-70,-55])||enemynear,vel y=0)
triggerall=(p2bodydist x =[15,70]) && var(20)<=60 && power >= 3000 && random < (350 * (AIlevel ** 2 / 64.0))
trigger1=ctrl || StateNo=40 || StateNo=52 ||stateno=21 || (StateNo=[100,103])
trigger2=var(6)|| var(7)
;----------------------------------------------------------------------
[State -1, Hashin Shou AI]
type=ChangeState
value=Ifelse(power>=2000 && random<=150,3350,3300)
triggerall=AILevel && numenemy && RoundState=2 && (stateno!=3300) && StateType != A && var(20)<=60 && power >= 1000 
triggerall=(enemynear,statetype != L) && (enemynear,stateno!=[5100,5220]) && (enemynear,stateno!=[120,155])
triggerall=(p2bodydist x =[15,70]) && (enemynear,statetype != A) && random < (200 * (AIlevel ** 2 / 64.0))
trigger1=ctrl || StateNo=40 || StateNo=52 || stateno=21 || (StateNo=[100,103])
trigger2=var(6)|| var(7)
;----------------------------------------------------------------------
[State -1, Yami Shigure AI]
type=ChangeState
value=Ifelse(power>=2000 && random<=100,3250,3200)
triggerall=AILevel && RoundState=2 && Numenemy && StateType != A && (stateno!=[3200,3220]) && var(20) <= 60 && power >= 1000 
triggerall=(p2bodydist x=[25,160]) && (enemynear,statetype=S) && enemynear,statetype!=L && !Numhelper(3205) 
triggerall=(enemynear,stateno!=[120,155]) && (enemynear,stateno!=[5100,5220]) && random < (100 * (AIlevel ** 2 / 64.0))
trigger1=ctrl || StateNo=40 || StateNo=52 ||stateno=21 || (StateNo=[100,103])
trigger2=var(6)|| var(7)
;----------------------------------------------------------------------
[State -1, Yoroi Doushi AI]
type=ChangeState
value=Ifelse(power>=2000 && random<=100,3050,3000)
triggerall=AILevel && RoundState=2 && Numenemy && StateType != A && (stateno!=[3000,3010]) && var(20) <= 60 && power >= 1000 
triggerall=(p2bodydist x=[25,160]) && (p2bodydist y = [ -80, 5]) && !(enemynear,canrecover) && random < (100 * (AIlevel ** 2 / 64.0))
triggerall=(enemynear,stateno!=[120,155]) && (enemynear,stateno!=[5100,5220]) && enemynear,statetype!=L 
trigger1=ctrl || StateNo=40 || StateNo=52 ||stateno=21 || (StateNo=[100,103])
trigger2=var(6)|| var(7)
;----------------------------------------------------------------------
[State -1, Kasumi Suzaku AI]
type=ChangeState
value=Ifelse(power>=2000 && random<=50,3150,3100)
triggerall=AILevel && numenemy && RoundState=2 && StateType = A && !Numhelper(3105) && (stateno!=3100) && Pos y<=-15 
triggerall=(enemynear,stateno!=[5120,5220]) && (p2bodydist x =[20,150]) && (p2bodydist y=[-80,255]) && (enemynear,Statetype!=L)
triggerall=var(20) <=60 && power >= 1000 && random < (100 * (AIlevel ** 2 / 64.0))
trigger1=ctrl
trigger2=var(6)|| var(7)
trigger3=(stateno=[600,650]) && movehit && random<250
trigger4=stateno=1410 && animelemtime(5)>0 
trigger5=stateno=1460 && Movecontact
trigger6=stateno=1600 && anim=1602 && animelemtime(4)>1
;=====================================================================
[State -1, Hien]
type=ChangeState
value=Ifelse(power>=1500 && random<=100,1450,1400)
triggerall=AILevel && numenemy && RoundState=2 && StateType != A && (enemynear,statetype!=L) && (enemynear,statetype!=A)  
triggerall=(p2bodydist x=[35,140]) && random < (150 * (AIlevel ** 2 / 64.0))
trigger1=ctrl || StateNo=40 || StateNo=52 ||stateno=21 || ((StateNo=[100,101])||stateno=103)
trigger2=var(5)
;----------------------------------------------------------------------
[State -1, Kazekiri]
type=Null;ChangeState
value=Ifelse(power>=2500 && random<=100,1250,1200)
value=Ifelse(power>=500 && enemynear,Movetype = A && random<100,1150,1100)
triggerall=AILevel && numenemy && RoundState=2 && StateType != A && (enemynear,statetype != L) && (enemynear,stateno!=[5100,5220]) && random < (100 * (AIlevel ** 2 / 64.0))
triggerall=(enemynear,stateno!=[120,155]) && (enemynear,statetype=A) && (p2bodydist x =[5,65]) && (p2bodydist y=[-130,5])
trigger1=ctrl || StateNo=40 || StateNo=52 ||stateno=21 || ((StateNo=[100,101])||stateno=103)
trigger2=var(5)

[State -1, Kazekiri]
type=ChangeState
value=Ifelse((power >= 500 && random < 100), 1250, 1200)
triggerall= AIlevel && numenemy
triggerall= roundstate=2 && statetype!=A && alive && !var(51) && prevstateno!=1200
triggerall= (p2bodydist x=[-25,50]) && (p2bodydist y=[-120,0]) && !(enemynear,stateno=[120,155])
triggerall= (enemynear,const(size.head.pos.y)<=-40) || (enemynear,statetype=A)
trigger1= ctrl || stateno=21 || (stateno=[100,101]) || (stateno=[120,131]) ;|| (stateno=200 || stateno=230 || stateno=245)
trigger1= enemynear,movetype=A && random<(ifelse(((stateno=5120 || stateno=5201) && !animtime || (var(14)=[1,2]) || enemynear,stateno!=var(57)),500,250))*(AIlevel**2/64.0)
trigger2= ((stateno=[210,215]) || stateno=225 || stateno=410 || stateno=440)
trigger2= movehit && p2bodydist x<=15 && random<150*(AIlevel**2/64.0)
trigger3= stateno=210 && anim=211 && animelemtime(4)<0 || stateno=240 && animelemtime(5)<0
trigger3= enemynear,statetype=A && movehit && (p2bodydist x=[-15,15]) && random<250*(AIlevel**2/64.0)
trigger4= (ctrl || stateno=0 || stateno=21 || (stateno=[100,101])) && enemynear,stateno=5210
trigger4= p2bodydist x<=30 && (p2dist y=[-90,0]) && random<400*(AIlevel**2/64.0)
trigger5=var(5)

;----------------------------------------------------------------------
[State -1, Raida AI]
type=ChangeState
value=Ifelse(power>=1500 && random<=100,1550,1500)
triggerall=AILevel && numenemy && RoundState=2 && StateType != A && (p2bodydist x =[0,20])
triggerall=enemynear,statetype != A && enemynear,statetype != L && random < (150 * (AIlevel ** 2 / 64.0))
trigger1=ctrl || StateNo=40 || StateNo=52 ||stateno=21 || ((StateNo=[100,101])||stateno=103)
trigger2=var(5) 
;----------------------------------------------------------------------
[State -1, Kiobori AI]
type=ChangeState
value=Ifelse(power>=1500 && random<=150,1150,1100)
triggerall=AILevel && numenemy && RoundState=2 && StateType != A && (enemynear,stateno!=[5100,5220]) 
triggerall=(enemynear,stateno!=[150,155]) && (enemynear,statetype=S) && (enemynear,Movetype != A) 
triggerall=(p2bodydist x=[42,160]) && var(45) && random < (150 * (AIlevel ** 2 / 64.0))
trigger1=ctrl || StateNo=40 || StateNo=52 ||stateno=21 || ((StateNo=[100,101])||stateno=103)
trigger2=var(5) && movehit && random<200
;----------------------------------------------------------------------
[State -1, Air Kunai AI]
type=ChangeState
value=Ifelse(power>=1500 && random<=250,1075,1050)
triggerall=AILevel && numenemy && RoundState=2 && StateType = A && Pos y<=-15  && Ifelse(!var(20),!Numhelper(1025),Numhelper(1025)<15)
triggerall=(enemynear,statetype != C) && (enemynear,statetype != L) && (enemynear,stateno!=[5120,5220]) && (enemynear,stateno!=[120,155])
triggerall=(p2bodydist x=[16,320])  && (p2bodydist y=[-60,125]) && random < (250 * (AIlevel ** 2 / 64.0)) 
trigger1=ctrl
trigger2=var(5)
trigger3=(stateno=[600,650]) && movecontact
trigger4=stateno=1410 && animelemtime(4)>2 
trigger5=stateno=1460 && Movecontact
trigger6=stateno=1600 && anim=1602 && animelemtime(4)>1
;----------------------------------------------------------------------
[State -1, Kunai AI]
type=ChangeState
value=Ifelse(power>=500 && random<100,1025,1000)
triggerall=AILevel && numenemy && RoundState=2 && StateType != A && Ifelse(!var(20),!Numhelper(1005),Numhelper(1005)<15)
triggerall=enemynear,statetype != L && (enemynear,stateno!=[5120,5220]) && (enemynear,stateno!=[120,155])
triggerall=enemynear,statetype != A && (p2bodydist x=[70,320]) && random < (100 * (AIlevel ** 2 / 64.0)) 
triggerall=var(45)=0
trigger1=ctrl || StateNo=40 || StateNo=52 ||stateno=21 || ((StateNo=[100,101])||stateno=103)
trigger2=var(5)
;----------------------------------------------------------------------
[State -1, Tsumuji AI]
type=ChangeState
value=Ifelse(power>=1500 && random<=250,1350,1300)
triggerall=AILevel && numenemy && RoundState=2 && StateType != A && random < (250 * (AIlevel ** 2 / 64.0))
triggerall=(enemynear,stateno!=[120,155]) && (enemynear,statetype!=L) && (enemynear,statetype!=A) && (enemynear,Movetype != A) && (p2bodydist x=[-50,90])
trigger1=ctrl || StateNo=40 || StateNo=52 ||stateno=21 || ((StateNo=[100,101])||stateno=103)
trigger2=var(5) && movehit && random<500
;----------------------------------------------------------------------
[State -1, Kasumigake AI]
type=ChangeState
value=1600
triggerall=AILevel && numenemy && RoundState=2 && StateType != A 
triggerall=(p2bodydist x >-80) && (enemynear,statetype!=L) && random < (75 * (AIlevel ** 2 / 64.0)) 
trigger1=ctrl || StateNo=40 || StateNo=52 ||stateno=21 || ((StateNo=[100,101])||stateno=103)
trigger2=var(5)
