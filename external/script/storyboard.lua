	...
	for k, v in ipairs(t.sceneOrder) do
		if k >= t.scenedef.startscene then
			...
			for i = 0, scene.end_time do
				...
				--play bgm
				if (i == 0 and (k - 1 == t.scenedef.startscene or scene.bgm ~= '') and ((attract and main.credits == 0) or matchno() > 1 or matchno() == -1)) and (winnerteam() == 1 or (attract and main.credits == 0)) then --EDIT prevent Intro storyboards to play music in worldtour
