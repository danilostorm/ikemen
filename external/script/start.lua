...
function start.f_unifySettings(t, t_chars)
	...
	for i = 1, #t do --for each order number
		if t_chars[i] ~= nil then --only if there are any characters available with this order
			...
			for j = 1, num do --iterate up to max amount of matches vs characters with this order
...
start.t_aiRampData.arcade = function()
	if start.p[2].numSimul == 2 then --EDIT: Simul 2
...
--draw portraits -EDITED FOR TEAMCOOP
function start.f_drawPortraits(t_portraits, side, t, subname, last, icon)
	...
	local member = 1
	if last and not main.coop then 
		member = #t_portraits
	end
	...
	if t['p' .. side .. subname .. '_num'] == 1 and last and main.coop then
		if t_portraits[#t_portraits].face2_data ~= nil then
        local v = t_portraits[#t_portraits]
        local x_offset = 173  -- You can adjust this offset value as needed

        -- Render the animation for the second character with an offset
        f_slideDistCalc(v.slide_dist, t['p' .. side .. '_member2' .. subname .. '_slide_dist'], t['p' .. side .. '_member2' .. subname .. '_slide_speed'])
        main.f_animPosDraw(
            v.face2_data,
            f_portraitsXCalc(side, t, subname, 2) + main.f_round(v.slide_dist[1]) + x_offset,
            t['p' .. side .. subname .. '_pos'][1] + (main.f_tableExists(t['p' .. side .. '_member2' .. subname .. '_offset'])[2] or 0) + main.f_round(v.slide_dist[2]),
            t['p' .. side .. subname .. '_face2_facing'],
            true
        )
    end	
    if t_portraits[#t_portraits].anim_data ~= nil then
        local v = t_portraits[#t_portraits]
        local x_offset = 240  -- You can adjust this offset value as needed
        -- Render the animation for the second character with an offset
        f_slideDistCalc(v.slide_dist, t['p' .. side .. '_member2' .. subname .. '_slide_dist'], t['p' .. side .. '_member2' .. subname .. '_slide_speed'])
        main.f_animPosDraw(
            v.anim_data,
            f_portraitsXCalc(side, t, subname, 1) + main.f_round(v.slide_dist[1]) + x_offset,
            t['p' .. side .. subname .. '_pos'][1] + t['p' .. side .. subname .. '_offset'][2] + (main.f_tableExists(t['p' .. side .. '_member2' .. subname .. '_offset'])[2] or 0) + main.f_round(v.slide_dist[2]),
            t['p' .. side .. subname .. '_facing'],
            f_portraitsXCalc(side, t, subname, 2) + main.f_round(v.slide_dist[2]) + x_offset,
            t['p' .. side .. subname .. '_pos'][2] + t['p' .. side .. subname .. '_offset'][2] + (main.f_tableExists(t['p' .. side .. '_member12' .. subname .. '_offset'])[2] or 0) + main.f_round(v.slide_dist[2]),
            t['p' .. side .. subname .. '_facing'],			
            true
        )
    end	
...
function start.f_cellMovement(selX, selY, cmd, side, snd, dir)
	...
	if main.f_input({cmd}, {'$U'}) or dir == 'U' then
		for i = 1, motif.select_info.rows do
			...
			elseif (start.t_grid[selY + 1][selX + 1].char ~= nil or motif.select_info.moveoveremptyboxes == 1) and start.t_grid[selY + 1][selX + 1].skip ~= 1 then
				break
	...
	elseif main.f_input({cmd}, {'$B'}) or dir == 'B' then
		if dir ~= nil then
			...
			for i = 1, motif.select_info.columns do
				...
				if (start.t_grid[selY + 1][selX + 1].char ~= nil or motif.select_info.moveoveremptyboxes == 1) and start.t_grid[selY + 1][selX + 1].skip ~= 1 then
					break
				end
			end
		end
		...
		for _, cmdType in ipairs({'select', 'next', 'previous'}) do
			if main.t_selGrid[cell][cmdType] ~= nil then
				for k, v in pairs(main.t_selGrid[cell][cmdType]) do
					...
					if main.f_btnPalNo(cmd) > 0 and (t_reservedChars[side][start.t_grid[y + 1][x + 1].char_ref]) and not cmdtype ~= 'select' and not cmdtype ~= 'previous' and not cmdtype ~= 'next' then
			sndPlay(motif.files.snd_data, motif.select_info['p' .. side .. '_cancel_snd'][1], motif.select_info['p' .. side .. '_cancel_snd'][2])
			end
	end
...
function start.f_selectMode()
	...
	while true do
		...
		if gamemode('vs100kumite') or gamemode('worldtour') or gamemode('timeattack') then -- use ratiopath for mission modes
			if start.f_getCharData(start.p[1].t_selected[1].ref).ratiopath ~= '' then
...
function start.f_selectReset(hardReset)
	...
	for side = 1, 2 do
		if hardReset then
			if main.coop then
			start.p[side].numSimul = 2 --math.max(2, config.NumSimul[1])
			end
			if not main.coop then
			start.p[side].numSimul = 1 --math.max(2, config.NumSimul[1])
			end
	...
	elseif not start.p[side].selEnd then
		...
		if selectState == 0 then
			...
			if start.p[side].t_selTemp[member] == nil then
				...
				if start.f_selGrid(start.c[player].cell + 1).char ~= 'randomselect' then
				start.p[side].t_selTemp[member].anim_data = changeColorPalette(start.p[side].t_selTemp[member].anim_data, 1)
				end
					...
					if motif.select_info.paletteselect >= 1 then
						...
						if palleteCounter > 1 then
							if motif.select_info.p1_face_anim >= 0 or motif.select_info['p' .. side .. '_member' .. member .. '_face_anim'] ~= nil then
								...
								start.p[side].t_selTemp[member].face2_data = createUniqueAnim(start.p[side].t_selTemp[member].face2_data)
								start.p[side].t_selTemp[member].face2_data = changeColorPalette(start.p[side].t_selTemp[member].face2_data, start.p[side].t_selTemp[member].pal, start.p[side].t_selTemp[member].pal)								
							end
						end
	...
	while true do
		...
		for side = 1, 2 do
			for member, v in ipairs(start.p[side].t_selected) do				
				...
				start.f_drawPortraits(main.f_remapTable(start.p[side].t_selTemp, start.t_orderRemap[member]), side, motif.vs_screen, '', false, t_icon[side])
				end
			end
		end
		--draw order values
...
--loading loop called after vs screen is finished
...
start.t_resultData.arcade = function()
	...
	-- EDITED TO USE IT AS A RESULT SCREEN FOR ARCADE 
...
start.t_resultData.survival = function()
	...
	if start.winCnt < 10 then --main.resultsTable.roundstowin and matchno() < #start.t_roster then
		return false 
		--start.t_result.stateType = ''
		--start.t_result.txt = false
		--start.t_result.winBgm = false
		--start.t_result.winbgdef = false
		--clearAllSound()
		--toggleNoSound(true)
...
local txt_matchno = main.f_createTextImg(motif.victory_screen, 'matchno')
local overlay_matchno = main.f_createOverlay(motif.victory_screen, 'overlay')
local t_txt_winquoteName = {}
for i = 1, 2 do
	table.insert(t_txt_winquoteName, main.f_createTextImg(motif.victory_screen, 'p' .. i .. '_name'))
end

function start.f_getMatchNumber()
    return matchno()
end
...
function start.f_victoryInit()
    local text = main.f_extractText(motif.victory_screen.matchno_text, start.f_getMatchNumber())
    txt_matchno:update({text = text[1]})
	...
	if winnerteam() < 1 or not main.victoryScreen or motif.victory_screen.enabled == 0 then
		return false
	elseif (matchno() > 9) then -- ((matchno() > 9) and ((t ~= nil and t[item].itemname == 'arcade') or (t == nil and not main.teamarcade))) or matchno() > 29 then
		return false
	elseif gamemode('versus') or gamemode('watch') or gamemode('netplayversus') then
...
function start.f_victory()
	...
	--frame transition
	if not start.t_victory.escFlag and (counter >= motif.victory_screen.time or esc() or main.f_input(main.t_players, {'m'})) then
		esc(false)
		main.f_fadeReset('fadeout', motif.victory_screen)
...
function start.f_hiscore(t, playMusic, place, infinite)
	...
	--draw fadein / fadeout
	if main.fadeType == 'fadein' and not main.fadeActive and not start.t_hiscore.input and (((not infinite and start.t_hiscore.counter >= motif.hiscore_info.time) or main.f_input(main.t_players, {'pal', 's'}))) then
...
function start.f_stageMusic()
	...
	if roundstate() == 2 then
		...
		if start.bgmround ~= roundno() then
			...
			-- stop vs screen track or life bgm even if stage music is not assigned
	...
	-- bgmusic.victory
elseif #start.t_music.musicvictory > 0 and start.bgmstate ~= -1 and roundstate() == 4 then
    local playMusic = false
    if gamemode('timeattack') and gamemode('vs100kumite') and gamemode('survival') then
        playMusic = true
    end

    if playMusic then
        for i = 1, 2 do
            if start.t_music.musicvictory[i] ~= nil and player(i) and win() and (roundtype() == 1 or roundtype() == 3) then
