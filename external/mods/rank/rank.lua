...
local function f_rank()
	...
	if gamemode('timeattack') or gamemode('watch') or gamemode('demo') or gamemode('versus') or gamemode('vs100kumite') or gamemode('freebattle') then
		active = false
		return
	end
	for side = 1, 2 do
		...
		if win() or drawgame() then
			if counter >= motif.rank_info['p' .. side .. '_removetime'] then
			rankInit = false
			tableremoved = true
			end
			-- Draw bg
			...
			if counter >= motif.rank_info['p' .. side .. '_score_displaytime'] then
				txt:draw()
				local score = math.min(scoretotal(), scoretotal() - score() + math.ceil(score() / math.max(1, 180 - counter - motif.rank_info['p' .. side .. '_score_displaytime']))) --EDIT - 90 to 180
	...
	if not paused() then
		if winnerteam() == 1 and player(winnerteam()) and counter == motif.rank_info.snd_time then
...
local function f_loop()
	if roundstart() then
		rankInit = false
		tableremoved = false
	elseif winnerteam() ~= -1 and player(winnerteam()) and stateno() < 190 and stateno() > 179 and roundstate() == 4 and tableremoved == false then
		f_rank()
	end
end
